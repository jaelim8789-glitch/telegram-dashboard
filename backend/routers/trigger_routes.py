...
async def list_rules(user: dict = Depends(get_current_user)):
    ...
    conn = _get_conn()
    try:
        rows = conn.execute("SELECT * FROM rules WHERE user_id=? ORDER BY created_at DESC", (user["id"],)).fetchall()
...
@router.post("")
async def create_rule(body: dict, user: dict = Depends(get_current_user)):
    _ensure_db()
    ...
    try:
        conn.execute(
            ...
            (rule_id, user["id"], body.get("name","Rule")[:200], body.get("description","")[:500],
...
@router.get("/{rule_id}")
async def get_rule(rule_id: str, user: dict = Depends(get_current_user)):
    _ensure_db()
    conn = _get_conn()
    try:
        r = conn.execute("SELECT * FROM rules WHERE id=? AND user_id=?", (rule_id, user["id"])).fetchone()
        if not r:
...
@router.put("/{rule_id}")
async def update_rule(rule_id: str, body: dict, user: dict = Depends(get_current_user)):
    _ensure_db()
    now = datetime.now(timezone.utc).isoformat()
    updates, params = [], []
    for field in body:
    ...
    params.append(now); params.append(rule_id); params.append(user["id"])
    conn = _get_conn()
    try:
...
async def delete_rule(rule_id: str, user: dict = Depends(get_current_user)):
    ...
    conn = _get_conn()
    try:
        conn.execute("DELETE FROM rules WHERE id=? AND user_id=?", (rule_id, user["id"]))
        conn.commit()
        return {"deleted": True}
    finally:
        conn.close()
...
async def trigger_stats(user: dict = Depends(get_current_user)):
    _ensure_db()
    conn = _get_conn()
    try:
        total = conn.execute("SELECT COUNT(*) as c FROM rules WHERE user_id=?", (user["id"],)).fetchone()["c"]
        active = conn.execute("SELECT COUNT(*) as c FROM rules WHERE user_id=? AND is_active=1", (user["id"],)).fetchone()["c"]

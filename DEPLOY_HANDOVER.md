...
- **신규 VPS SSH (Lightnode)**: `ssh telemon-lightnode` (SSH config alias 등록, 130.94.30.112 / root) - *SSH 연결 문제로 현재 미사용*
...
SSH Config 파일 (`~/.ssh/config`)에 다음 항목을 추가:

```
Host telemon-vps
    HostName 130.94.32.152
    User root
...
Host telemon-lightnode
    ...
    User root
    IdentityFile ~/.ssh/telemon_vps_key
    # Cloudflare Zero Trust 문제로 현재 미사용
```
...
- **신규 VPS (130.94.30.112) SSH 연결 문제**: Cloudflare Zero Trust 또는 방화벽 설정으로 인한 연결 실패. 해결을 위한 절차는 `docs/vps-connection-troubleshooting.md` 참고

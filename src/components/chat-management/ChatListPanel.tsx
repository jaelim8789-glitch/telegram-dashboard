...
export function ChatListPanel({
  ...
  const roomMenuTarget = roomMenu ? rooms.find((r) => r.id === roomMenu.roomId) : null;
  ...
  function handlePinFromMenu(roomId: string) {
    onTogglePin(roomId);
    setRoomMenu(null);
  }
          ...
          <AnimatePresence mode="popLayout">
            {pinned.length > 0 && !search && (
              <>
                ...
                <div className="mx-3 my-1 h-px bg-violet-500/10" />
              </>
            )}
            {filtered.map((room) => (
              <RoomItem

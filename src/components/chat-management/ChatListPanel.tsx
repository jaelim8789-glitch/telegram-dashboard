      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin">
        {filtered.length === 0 ? (
          <div className="flex h-full items-center justify-center p-4">
            <p className="text-sm text-app-text-muted">대화가 없습니다</p>
          </div>
        ) : (
          filtered.map((room) => (
            <button
              ...
              className={`flex w-full items-center gap-3 px-3 py-2.5 text-left transition-colors ${
                activeRoomId === room.id
                  ...
                  : "border-l-2 border-transparent hover:bg-app-card-hover"
              }`}
            >
              <Avatar name={room.name} />
              <div className="min-w-0 flex-1">
                <div className="flex items-center justify-between">
                  <span className="truncate text-sm font-medium text-app-text">{room.name}</span>
                  <span className="shrink-0 text-[10px] text-app-text-subtle">
                    {formatTime(room.lastMessageTime)}
                  </span>
                </div>
                <div className="flex items-center justify-between gap-1">
                  <span className="truncate text-xs text-app-text-muted">{room.lastMessage}</span>
                  {room.unreadCount > 0 && (
                    <span className="flex h-4 min-w-4 shrink-0 items-center justify-center rounded-full bg-violet-500 px-1 text-[10px] font-bold text-white">

...
const STATUS_STYLE: Record<Account["status"], { dot: string; label: string }> = {
  active: { dot: "bg-app-success", label: "?œì„±" },
  inactive: { dot: "bg-app-text-subtle", label: "ë¹„í™œ?? },
  banned: { dot: "bg-app-danger", label: "ì°¨ë‹¨?? },
  suspended: { dot: "bg-app-warning", label: "?•ì??? },
};

const HEALTH_ICON: Record<AccountHealthState, { icon: typeof AlertTriangle; color: string; title: string }> = {
  healthy: { icon: CheckCircle2, color: "text-app-success", title: "?•ìƒ" },
  unauthorized: { icon: Plug, color: "text-app-warning", title: "?¸ì…˜ ë§Œë£Œ - ?¬ì¸ì¦??„ìš”" },
  banned: { icon: Ban, color: "text-app-danger", title: "ì°¨ë‹¨?? },
  restricted: { icon: ShieldAlert, color: "text-app-danger", title: "ê·¸ë£¹ ë°œì†¡ ?œí•œ - ?”ë ˆê·¸ëž¨ ?œìž¬ ?˜ì‹¬" },
  rate_limited: { icon: Clock, color: "text-app-warning", title: "?œí•œ ì´ˆê³¼" },
  error: { icon: ShieldAlert, color: "text-app-danger", title: "ë°œì†¡ ?¤ë¥˜" },
  unknown: { icon: AlertTriangle, color: "text-app-text-muted", title: "?íƒœ ë¯¸í™•?? },
  not_configured: { icon: WifiOff, color: "text-app-text-subtle", title: "?¸ì…˜ ?†ìŒ - ?±ë¡ ?„ìš”" },
};
...
export function AccountCard({ account, selected, health, lastError, isFavorite, groupFilter, onSelect, onDelete, onToggleFavorite, onClearError, onResume }: AccountCardProps) {
  ...
  function handleEditLabel(e: React.MouseEvent) {
    ...
    const newLabel = window.prompt("ê³„ì • ë³„ì¹­???…ë ¥?˜ì„¸??(ë¹„ìš°ë©??? œ)", current);
  ...
  return (
    <>
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="truncate text-xs text-app-text-muted">
            {label ? `${getAccountDisplayName(account)} Â· ${account.phone}` : account.phone}
          </div>
        </div>
          ...
          {groupPickerOpen && (
            <>
              ...
              <div className="fixed inset-x-4 bottom-4 z-40 sm:absolute sm:inset-x-auto sm:right-0 sm:top-full sm:mt-1 sm:w-48 rounded-xl border border-app-border bg-app-surface p-1.5 shadow-xl">
                <p className="mb-1 px-2 text-[10px] font-semibold uppercase tracking-wider text-app-text-muted">ê·¸ë£¹ ì§€??/p>
                {allGroups.length === 0 && (
                  <p className="px-2 py-2 text-[11px] text-app-text-muted italic">ê·¸ë£¹???†ìŠµ?ˆë‹¤</p>
                )}
                {allGroups.map((g) => {
        ...
        {onToggleFavorite && (
          <button
            type="button"
            title={isFavorite ? "ì¦ê²¨ì°¾ê¸° ?œê±°" : "ì¦ê²¨ì°¾ê¸° ì¶”ê?"}
        ...
        <button
          type="button" title="?? œ" onClick={(e) => { e.stopPropagation(); setConfirmOpen(true); }} disabled={deleting}
          className={cn(
      ...
      <ConfirmDialog
        open={confirmOpen}
        title={`${getAccountDisplayName(account)} ê³„ì •???? œ? ê¹Œ??`}
        variant="danger"
        confirmLabel="?? œ"
        cancelLabel="ì·¨ì†Œ"

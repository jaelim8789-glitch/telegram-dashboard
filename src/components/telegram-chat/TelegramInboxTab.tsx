...
export function TelegramInboxTab() {
  ...
  const loadDialogs = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("?€?”ë°©??ë¶ˆëŸ¬?????†ìŠµ?ˆë‹¤");
      const data = await res.json();
      setDialogs(data);
    } catch (e) {
      setError(e instanceof Error ? e.message : "ë¡œë“œ ?¤íŒ¨");
    } finally {
      setLoading(false);
    }
  }, [activeAccountId]);
  ...
  return (
    <div className="flex h-[calc(100dvh-8rem)]">
      ...
      <div className={cn(
        ...
        <div className="flex items-center justify-between px-3 py-2.5 border-b border-app-border">
          <div>
            ...
            <p className="text-[10px] text-app-text-muted">
              {dialogs.length}ê°??€?”ë°©{totalUnread > 0 && ` Â· ${totalUnread}ê°????½ìŒ`}
            </p>
          </div>
        ...
        {error ? (
          <div className="flex-1 flex items-center justify-center p-4">
            <InlineError action={<button onClick={loadDialogs} className="text-xs underline hover:no-underline">?¤ì‹œ ?œë„</button>}>{error}</InlineError>
          </div>
        ) : (
          <DialogList
      ...
      <div className={cn(
        ...
        {activeChatId && activeAccountId ? (
          <TelegramChatView
            ...
            chatId={activeChatId}
            chatTitle={activeChat?.title || "ì±„íŒ…"}
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-app-text-muted">
            ...
            <h3 className="text-base font-semibold text-app-text mb-1">Telegram ë©”ì‹œì§€</h3>
            <p className="text-xs text-center max-w-xs">
              ?¼ìª½?ì„œ ?€?”ë°©??? íƒ?˜ê±°??ê²€?‰í•˜??ë©”ì‹œì§€ë¥??•ì¸?˜ì„¸??
            </p>
          </div>
        )}
      </div>
      <ConfirmDialog
        ...
        title="?€?”ë°© ?? œ"
        description="?•ë§ë¡????€?”ë°©???? œ?˜ì‹œê² ìŠµ?ˆê¹Œ? ëª¨ë“  ë©”ì‹œì§€ê°€ ?êµ¬?ìœ¼ë¡??? œ?©ë‹ˆ??"
        confirmLabel="?? œ"
        variant="danger"
      />
    </div>

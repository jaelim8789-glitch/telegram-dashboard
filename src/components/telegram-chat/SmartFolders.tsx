...
export function SmartFolders({ onSelectFolder, dialogs = [] }: SmartFoldersProps) {
  ...
  useEffect(() => {
    fetch(`${API_BASE}/api/smart-folders/rules`, { headers: authHeaders })
      .then((r) => r.json())
      .then(setRules)
      .catch((e) => { console.error("[SmartFolders] smart-folders rules fetch ?§Ìå®", e); toast("error", "?§Îßà???¥Îçî Í∑úÏπô??Î∂àÎü¨?§Ï? Î™ªÌñà?µÎãà??); });
  }, []);
  ...
  const addRule = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast("success", "?¥Îçî Í∑úÏπô??Ï∂îÍ??òÏóà?µÎãà??);
      }
    } catch (e) { console.warn('Unhandled error in SmartFolders', e) }
  };
    ...
    <div className="rounded-2xl border border-app-border bg-app-card p-3">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-semibold text-app-text">?§Îßà???¥Îçî</span>
          <span className="text-[9px] text-app-text-muted">({totalCount}Í∞??Ä??</span>
        </div>
        ...
        {showAddRule && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden mb-2">
            <div className="space-y-1.5 rounded-xl bg-app-card-hover p-2">
              <input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="?¥Îçî ?¥Î¶Ñ (?? Í≥†Í∞ùÎ¨∏Ïùò)"
              ...
              <input value={newKeywords} onChange={(e) => setNewKeywords(e.target.value)} placeholder="?§Ïõå??(?ºÌëúÎ°?Íµ¨Î∂Ñ, ?? Î¨∏Ïùò,Í≤¨Ï†Å,Í∞ÄÍ≤?"
                className="w-full rounded-lg border border-app-border bg-app-bg px-2 py-1.5 text-[10px] text-app-text placeholder:text-app-text-muted focus:outline-none focus:border-app-primary" />
      ...
      <div className="space-y-0.5 max-h-48 overflow-y-auto">
        {Object.entries(categories).map(([name, items]) => {
          if (items.length === 0 && name === "Í∏∞Ì?") return null;
          return (
            ...
            >
              <span className="text-xs">{rules.find((r) => r.name === name)?.icon || "?ìÅ"}</span>

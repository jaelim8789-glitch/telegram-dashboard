        ...
        >
          <div className="rounded-2xl border border-app-border bg-app-card p-4 shadow-2xl">
            ...
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-sm font-bold text-app-text">TeleMon???��????�셨?�요?</span>
            </div>
            <p className="text-xs text-app-text-muted mb-3">?�스?�어 리뷰�??�겨주시�????��? ?�비?��? ?�니??/p>
            <div className="flex gap-2">
              <button onClick={rate} className="flex-1 rounded-xl bg-app-primary py-2 text-xs font-semibold text-white hover:opacity-90 flex items-center justify-center gap-1">
                <Star className="h-3.5 w-3.5" /> 리뷰 ?�기�?<ExternalLink className="h-3 w-3" />
              </button>
              <button onClick={later} className="rounded-xl border border-app-border px-4 py-2 text-xs text-app-text-muted hover:bg-app-card-hover">?�음??/button>
            </div>
          </div>
        </motion.div>
      )}

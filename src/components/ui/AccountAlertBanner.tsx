    ...
    const check = async () => {
      try {
        ...
        const issues = list.filter((a: any) => a.status === "error" || a.status === "banned" || a.status === "limited")
          .map((a: any) => ({ id: a.id, phone: a.phone, issue: a.status === "banned" ? "ì°¨ë‹¨?? : a.status === "limited" ? "?œí•œ?? : "?¤ë¥˜" }));
        setAlerts(issues);
        ...
        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }}
          ...
          <div className="flex items-center justify-between px-4 py-2">
            <div className="flex items-center gap-2 min-w-0">
              ...
              {visible.length > 1 && <span className="text-[10px] text-white/70 shrink-0">??{visible.length - 1}ê°?/span>}
            </div>

...
export function VoiceMemoRecorder({ onTranscribe }: { onTranscribe: (text: string) => void }) {
  ...
  return (
    <button onClick={toggle} className={cn("flex h-10 w-10 items-center justify-center rounded-full transition-all active:scale-90", recording ? "bg-red-500 animate-pulse" : "border border-app-border text-app-text-muted hover:text-app-text")} aria-label={recording ? "?¹ìŒ ì¤‘ì?" : "?Œì„± ë©”ëª¨"}>

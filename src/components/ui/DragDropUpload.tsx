...
export function DragDropUpload({
  ...
  const processFiles = (newFiles: File[]) => {
    ...
    if (!multiple && files.length + newFiles.length > 1) {
      errors.push('단일 파일만 업로드할 수 있습니다.');
      setErrors(errors);
      return;
    }

    if (multiple && files.length + newFiles.length > maxFiles) {
      errors.push(`${maxFiles}개까지 파일을 업로드할 수 있습니다.`);
      setErrors(errors);
      return;
    }
    ...
    for (const file of newFiles) {
      ...
      if (file.size > maxFileSize) {
        errors.push(`"${file.name}" 파일이 너무 큽니다. (${(file.size / 1024 / 1024).toFixed(2)}MB > ${(maxFileSize / 1024 / 1024).toFixed(2)}MB)`);
        continue;
      }
  ...
  return (
    <div className={cn("border-2 border-dashed rounded-lg p-6 text-center transition-colors", {
      ...
      >
        ...
        <p className="text-lg font-medium mb-1">파일을 여기에 끌어다 놓거나 클릭하세요</p>
        <p className="text-sm text-gray-500 mb-3">
          {accept === '*' ? '모든 형식' : accept} 파일, 최대 {(maxFileSize / 1024 / 1024).toFixed(0)}MB
      ...
      {files.length > 0 && (
        <div className="mt-4">
          <div className="flex justify-between items-center mb-2">
            <h4 className="font-medium">선택된 파일</h4>
            <Button
              type="button"
              variant="ghost"
              size="sm"

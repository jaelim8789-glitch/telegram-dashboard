          ...
          <>
            ...
            >
              <div className="flex items-center justify-between px-4 py-3 border-b border-app-border/60">
                <h3 className="text-sm font-semibold text-app-text">?åÎ¶º</h3>
                <div className="flex items-center gap-1.5">
                            ...
                            >
                              {disabled ? (
                                <>
                                  ...
                                  <span>Î∏åÎùº?∞Ï? ?åÎ¶º ÏºúÍ∏∞</span>
                                </>
                              ) : (
                                <>
                                  <BellOff className="h-3.5 w-3.5 text-app-text-muted" />
                                  <span>Î∏åÎùº?∞Ï? ?åÎ¶º ?ÑÍ∏∞</span>
                                </>
                              )}
                            </button>
                          </motion.div>
                  ...
                  {unread > 0 && (
                    <button onClick={markAllRead} className="text-xs text-app-primary hover:underline">Î™®Îëê ?ΩÏùå</button>
                  )}
                  {notifState.length > 0 && (
                    <button onClick={clearAll} className="p-1 text-app-text-muted hover:text-app-danger transition-colors" title="?ÑÏ≤¥ ??†ú">
              ...
              <div className="max-h-80 overflow-y-auto">
                {notifState.length === 0 ? (
                  <div className="py-8 text-center text-xs text-app-text-muted">?åÎ¶º???ÜÏäµ?àÎã§</div>
                ) : (
                  notifState.map((n) => {

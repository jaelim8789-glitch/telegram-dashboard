...
const DEFAULTS: Shortcut[] = [
  { key: "/hi", expansion: "?ˆë…•?˜ì„¸?? TeleMon?…ë‹ˆ??" },
  { key: "/evt", expansion: "[?´ë²¤???ˆë‚´] ì§€ê¸?ë°”ë¡œ ?•ì¸?´ë³´?¸ìš”!" },
  { key: "/thx", expansion: "ê°ì‚¬?©ë‹ˆ?? ì¢‹ì? ?˜ë£¨ ?˜ì„¸??" },
];
...
export function ShortcutEditor() {
  ...
  return (
    <div className="space-y-2">
      <p className="text-xs font-semibold text-app-text">?¨ì¶•??/p>
      {shortcuts.map(s => (
        <div key={s.key} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-app-card-hover">
          ...
          <button onClick={() => remove(s.key)} className="text-[10px] text-app-text-muted hover:text-app-danger">?? œ</button>
        </div>
      ))}
      <div className="flex gap-2">
        <input value={key} onChange={e => setKey(e.target.value)} placeholder="/?¤ì›Œ?? className="flex-1 rounded-lg border border-app-border bg-app-bg px-3 py-2 text-xs text-app-text outline-none" />
        <input value={exp} onChange={e => setExp(e.target.value)} placeholder="?•ì¥ ë¬¸êµ¬" className="flex-[2] rounded-lg border border-app-border bg-app-bg px-3 py-2 text-xs text-app-text outline-none" />
        <button onClick={handleAdd} className="rounded-lg bg-app-primary px-3 py-2 text-xs font-semibold text-white active:scale-95">ì¶”ê?</button>
      </div>
    </div>
  );
}

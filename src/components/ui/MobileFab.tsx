...
export function MobileFab() {
  ...
  const contextActions: { icon: React.ComponentType<{ className?: string }>; label: string; action: () => void }[] = navView === "chat"
    ? [
        ...
        { icon: PenLine, label: "새 메시지", action: () => {} },
        { icon: ChevronUp, label: "위로가기", action: handleScrollTop },
      ]
    : currentTab === "send"
    ? [
        { icon: Send, label: "새 발송", action: () => setActiveTab("send") },
        { icon: RefreshCw, label: "새로고침", action: () => { fetchAccounts(); } },
        { icon: ChevronUp, label: "위로가기", action: handleScrollTop },
      ]
    : [
        { icon: UserPlus, label: "계정 등록", action: () => setActiveTab("register") },
        { icon: RefreshCw, label: "새로고침", action: () => { fetchAccounts(); } },
        { icon: ChevronUp, label: "위로가기", action: handleScrollTop },
      ];
  ...
  return (
    <div ref={fabRef} className="fixed right-4 z-40 flex flex-col items-end" style={{ bottom: "calc(5rem + env(safe-area-inset-bottom, 0px))" }}>
      ...
      <motion.button
        ...
        aria-label={open ? "메뉴 닫기" : "빠른 작업 열기"}
      >

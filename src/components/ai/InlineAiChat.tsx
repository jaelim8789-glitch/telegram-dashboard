...
export function InlineAiChat() {
  ...
  const loadAgents = useCallback(async () => {
    ...
    try {
      ...
      setAgentsError(err instanceof Error ? err.message : "Agent Î™©Î°ù??Î∂àÎü¨?§Îäî???§Ìå®?àÏäµ?àÎã§.");
    } finally {
  ...
  // ?Ä?Ä Auto-init: create default agent + chat on first visit ?Ä?Ä?Ä?Ä?Ä?Ä
  useEffect(() => {
    ...
    async function autoInit() {
      ...
      // 1. If no agents exist, create a default "?òÎßå??AI" agent
      if (agents.length === 0) {
        try {
          const defaultAgent = await agentApi.createAgent({
            ...
            role: "assistant",
            systemPrompt:
              "?àÎäî TeleMon??AI ÎπÑÏÑú '?òÎßå??AI'?? ?¨Ïö©?êÎ? ÏπúÏ†à?òÍ≤å ?ÑÏ?Ï£ºÍ≥†, " +
              "TeleMon ?åÎû´???îÎ†àÍ∑∏Îû® ÎßàÏ????êÎèô????Í¥Ä??ÏßàÎ¨∏???ÑÎ¨∏?ÅÏúºÎ°??µÎ??¥Ï§ò. " +
              "?úÍµ≠?¥Î°ú ?ëÎãµ?òÍ≥†, ?ÑÏöî?????¥Î™®ÏßÄÎ•??ÅÏ†à???¨Ïö©??",
          });
          if (cancelled) return;
  ...
  useEffect(() => {
    ...
    agentApi.fetchChatMessages(activeChatId).then((list) => {
      ...
      if (!cancelled) setMessagesError(err instanceof Error ? err.message : "Î©îÏãúÏßÄÎ•?Î∂àÎü¨?§Îäî???§Ìå®?àÏäµ?àÎã§.");
    }).finally(() => {
  ...
  useEffect(() => {
    ...
    }).catch((e) => { console.error("[InlineAiChat] fetch agents summary ?§Ìå®", e); toast("error", "Î∞úÏÜ° ?îÏïΩ ?ïÎ≥¥Î•?Î∂àÎü¨?§Ï? Î™ªÌñà?µÎãà??); }).finally(() => {
  ...
  // ?Ä?Ä Quick prompts ?Ä?Ä
  const quickPrompts = [
    { icon: BarChart3, label: "?§Îäò Î∞úÏÜ°?ÑÌô©", text: "?§Îäò Î∞úÏÜ° ?ÑÌô©???îÏïΩ?¥Ï§ò" },
    { icon: AlertTriangle, label: "?§Ìå®??Î∞úÏÜ°", text: "ÏµúÍ∑º ?§Ìå®??Î∞úÏÜ° ?¥Ïó≠Í≥??êÏù∏???åÎ†§Ï§? },
    { icon: FileText, label: "ÏµúÍ∑º Î°úÍ∑∏", text: "ÏµúÍ∑º 24?úÍ∞Ñ Î∞úÏÜ° Î°úÍ∑∏Î•?Î∂ÑÏÑù?¥Ï§ò" },
    { icon: MessageSquare, label: "?µÏû• Ï∂îÏ≤ú", text: "?§Îäò ?§Ïñ¥??Î©îÏãúÏßÄ???Ä???µÏû•??Ï∂îÏ≤ú?¥Ï§ò" },
    { icon: Clock, label: "?àÏïΩ ?ÑÌô©", text: "?ÑÏû¨ ?àÏïΩ??Î∞úÏÜ° Î™©Î°ù??Î≥¥Ïó¨Ï§? },
  ];
  ...
  async function sendMessageWithInput(text: string) {
    ...
    fetchWithTimeout(`${BASE_URL}/api/ai/chats/${activeChatId}/message`, {
      ...
      if (!res.ok) {
        setStreamMsg({ role: "agent", content: "?úÎ≤Ñ ?§Î•òÍ∞Ä Î∞úÏÉù?àÏäµ?àÎã§.", tokensUsed: 0 });
        return;
      }
      const data = await res.json();
      if (data.pending_confirmation) setPendingConfirmation(data.pending_confirmation);
      if (data.level_up && data.new_level) {
        toast("success", `?éâ Lv.${data.new_level} ?¨ÏÑ±!`, {
          description: `${data.exp_gained || 0} EXPÎ•??çÎìù?àÏäµ?àÎã§.`,
          duration: 5000,
        });
        agentApi.fetchAgents().then(setAgents).catch((e) => { console.error("[InlineAiChat] fetchAgents Í∞±Ïã† ?§Ìå®", e); toast("error", "Agent Î™©Î°ù Í∞±Ïã†???§Ìå®?àÏäµ?àÎã§"); });
      }
      ...
      setStreamMsg({ role: "agent", content: "?§Ìä∏?åÌÅ¨ ?§Î•òÍ∞Ä Î∞úÏÉù?àÏäµ?àÎã§.", tokensUsed: 0 });
    }).finally(() => {
      setLoading(false);
  ...
  // ?Ä?Ä Voice input (long-press ??auto-send, short tap ??fill text) ?Ä?Ä
  useEffect(() => {
    ...
    if (SpeechRecognition) {
      ...
      recognition.onerror = () => {
        ...
        toastRef.current("error", "?åÏÑ± ?∏Ïãù ?§Î•ò");
      };
      recognition.onend = () => {
        setIsListening(false);
        isLongPressRef.current = false;
  ...
  async function handleConfirmTool() {
    ...
    try {
      ...
      const msgs = await agentApi.fetchChatMessages(activeChatId);
      setMessages(msgs);
      toast("success", `??${pendingConfirmation.label} ?§Ìñâ ?ÑÎ£å`, {
      ...
      toast("error", "?§Ìñâ ?§Ìå®", {
        description: err instanceof Error ? err.message : "?????ÜÎäî ?§Î•ò",
      });
    } finally {
    ...
    if (!activeAgentId) {
      return (
        <div className="flex flex-col items-center justify-center h-full gap-2 text-app-text-muted">
          ...
          <p className="text-xs">AgentÎ•??†ÌÉù?òÍ≥† ?Ä?îÎ? ?úÏûë?òÏÑ∏??/p>
        ...
        <div className="flex flex-col items-center justify-center h-full gap-2 text-app-text-muted">
          {autoChatCreating ? (
            <>
              ...
              <p className="text-xs">Ï±ÑÌåÖ Ï§ÄÎπ?Ï§?..</p>
            </>
          ) : (
            <>
              <MessageSquare className="h-8 w-8 opacity-30" />
              <p className="text-xs">
                <span className="font-medium text-app-text">{activeAgent?.name}</span> ?ëã
              </p>
              <Button variant="primary" size="sm" onClick={handleNewChat}>
                <Plus className="h-3 w-3" /> ??Ï±ÑÌåÖ
              </Button>
            </>
          )}
        </div>
          ...
          {messagesLoading ? (
            ...
            <div className="flex flex-col items-center gap-4 py-6">
              <div className="w-full max-w-sm rounded-2xl border border-app-primary/20 bg-gradient-to-br from-app-primary/5 to-app-bg p-5 text-center">
                ...
                <p className="text-sm font-bold text-app-text">{activeAgent?.name || "AI"} ?ëã</p>
                <p className="text-xs text-app-text-muted mt-1">Î¨¥Ïóá???ÑÏ??úÎ¶¥ÍπåÏöî? ?Ä?îÌïò??ÏßàÎ¨∏?¥Î≥¥?∏Ïöî</p>
                {broadcastSummary && (
                  <div className="flex items-center justify-center gap-2 mt-3 text-[10px]">
                    <span className="flex items-center gap-0.5 text-app-success"><CheckCircle className="h-3 w-3" />{broadcastSummary.sent}Í±??±Í≥µ</span>
                    {broadcastSummary.failed > 0 && (
                      <button onClick={() => handleQuickPrompt("?§Ìå®??Î∞úÏÜ° ?êÏù∏ Î∂ÑÏÑù?¥Ï§ò")} className="flex items-center gap-0.5 text-app-danger hover:underline">
                        <AlertTriangle className="h-3 w-3" />{broadcastSummary.failed}Í±??§Ìå®
                      </button>
                    )}
                    {broadcastSummary.scheduled > 0 && (
                      <span className="flex items-center gap-0.5 text-app-text-muted"><Clock className="h-3 w-3" />{broadcastSummary.scheduled}Í±??àÏïΩ</span>
                    )}
                  </div>
                )}
                <div className="flex items-center justify-center gap-2 mt-3">
                  <button onClick={() => handleQuickPrompt("?§Îäò Î∞úÏÜ° ?ÑÌô© ?îÏïΩ?¥Ï§ò")} className="rounded-full bg-app-primary/10 px-3 py-1.5 text-[11px] font-medium text-app-primary hover:bg-app-primary/20 transition-colors">?ìä ?§Îäò Î¶¨Ìè¨??/button>
                  <button onClick={() => useDashboardStore.getState().setActiveTab("send")} className="rounded-full border border-app-border px-3 py-1.5 text-[11px] text-app-text-muted hover:border-app-primary/30 hover:text-app-text transition-colors">?çÔ∏è ??Î∞úÏÜ° ?ëÏÑ±</button>
                </div>
              </div>
              <p className="text-[10px] text-app-text-muted -mb-2">?êÏ£º Î¨ªÎäî ÏßàÎ¨∏</p>
          ...
          {loading && !streamMsg?.content && (
            <div className="flex justify-start">
              <div className="flex items-center gap-2 rounded-2xl rounded-bl-md border border-app-border bg-app-card-hover px-3 py-2 text-xs text-app-text-muted">
                <Loader2 className="h-3.5 w-3.5 animate-spin" />?µÎ? ?ùÏÑ± Ï§?..
              </div>
            </div>
          )}
          {streamMsg?.content && (
  ...
  return (
    <div className="relative flex flex-col h-full min-h-0 sm:min-h-[400px] rounded-xl border border-app-border bg-app-card overflow-hidden">
        ...
        {agentsLoading ? (
          ...
          <div className="flex items-center gap-1.5 truncate">
            ...
            <button onClick={() => loadAgents()} className="shrink-0 flex items-center gap-1 rounded-md px-2 py-0.5 text-[10px] font-medium text-app-primary hover:bg-app-primary/10 transition-colors">
              <RefreshCw className="h-3 w-3" /> ?¨Ïãú??
            </button>
          </div>
        ) : agents.length === 0 ? (
          <span className="text-xs text-app-text-muted truncate">AgentÍ∞Ä ?ÜÏäµ?àÎã§</span>
        ) : (
          <>
            {agents.map((a) => (
              <button
      ...
      {/* ?Ä?Ä Chat List Side Panel (left slide-in overlay) ?Ä?Ä */}
      {chatPanelOpen && (
        <>
            ...
            <div className="flex-1 overflow-y-auto">
              {chatsLoading ? (
                ...
                <div className="flex flex-col items-center justify-center py-10 gap-2 text-app-text-muted">
                  ...
                  <p className="text-[11px]">?Ä??Í∏∞Î°ù???ÜÏäµ?àÎã§</p>
                </div>
              ) : (
      ...
      {activeChatId && broadcastSummary && !summaryLoading && (
        <div className="border-b border-app-border/40 bg-app-bg/30 px-3 py-2 shrink-0">
          <div className="flex items-center gap-3 text-[11px]">
            <span className="text-app-text-muted shrink-0">?§Îäò</span>
            <button
              onClick={() => handleQuickPrompt("?§Îäò Î∞úÏÜ° ?ÑÌô© ?ÅÏÑ∏???åÎ†§Ï§?)}
              className="flex items-center gap-1 text-app-success hover:underline"
            >
              <CheckCircle className="h-3 w-3" />{broadcastSummary.sent}Í±??±Í≥µ
            </button>
            {broadcastSummary.failed > 0 && (
              <button
                onClick={() => handleQuickPrompt("?§Ìå®??Î∞úÏÜ° ?êÏù∏ Î∂ÑÏÑù?¥Ï§ò")}
            ...
            {broadcastSummary.scheduled > 0 && (
              <span className="flex items-center gap-1 text-app-text-muted">
                <Clock className="h-3 w-3" />{broadcastSummary.scheduled}Í±??àÏïΩ
              </span>
            )}
          </div>
        </div>
            ...
            <div className="flex-1 min-w-0">
              ...
              {pendingConfirmation.arguments && (pendingConfirmation.arguments as any).account_id ? (
                <p className="text-[10px] text-app-text-muted mt-0.5">
                  Í≥ÑÏ†ï: {String((pendingConfirmation.arguments as any).account_id).slice(0, 30)}
                </p>
              ) : null}
            </div>
          ...
          <div className="border-t border-app-primary/20 bg-app-primary-muted/5 px-3 py-2 shrink-0">
            <div className="flex items-center gap-2 text-[11px]">
              ...
              <div className="min-w-0 flex-1">
                <span className="font-medium text-app-text">Î∞úÏÜ° Ï§ÄÎπ?Ï§?/span>
                {sendMsg.trim() && <span className="text-app-text-muted ml-1 truncate">&mdash; {sendMsg.slice(0, 30)}{sendMsg.length > 30 ? "..." : ""}</span>}
              </div>
              <span className="text-app-text-muted shrink-0">{sendGroups.length}Î™?/span>
              <button
                type="button"
              ...
              >
                Î∞úÏÜ°??<ExternalLink className="h-3 w-3" />
              </button>
            </div>
          </div>
        );
          ...
          <div className="flex gap-2">
            ...
            {speechSupported && (
              <button
                ...
                title="Í∏∏Í≤å ?ÑÎ•¥Î©?Î∞îÎ°ú ?ÑÏÜ°, ÏßßÍ≤å ?ÑÎ•¥Î©??çÏä§???ÖÎ†•"
              >
            ...
            <input
              ...
              placeholder={`${activeAgent?.name || "Agent"}?êÍ≤å Î©îÏãúÏßÄ Î≥¥ÎÇ¥Í∏?..`}
              disabled={loading}
      ...
      {/* ?Ä?Ä FAB (Floating Action Button) ?Ä?Ä */}

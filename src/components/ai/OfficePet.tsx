...
const PET_NAMES = ["?òÎπÑ", "Ï¥àÏΩî", "Î™ΩÏù¥", "Î£®ÌÇ§", "ÏΩîÏΩî"];
const PET_EMOJIS = ["?ê±", "?ê∂", "?ê∞", "?êπ", "?¶ä"];
...
function loadPet(): PetData {
  if (typeof window === "undefined") return { name: "?òÎπÑ", emoji: "?ê±", level: 1, exp: 0, fed: 0, petted: 0, lastFed: "" };
  try {
...
const PET_MOODS: Record<string, { emoji: string; msg: string }> = {
  happy: { emoji: "?òä", msg: "Í∏∞Î∂Ñ??Ï¢ãÏïÑ??" },
  hungry: { emoji: "?òã", msg: "Î∞∞Í≥†?åÏöî..." },
  sleepy: { emoji: "?ò¥", msg: "Ï°∏Î†§??.." },
  love: { emoji: "?•∞", msg: "?¨Îûë?¥Ïöî! ?íï" },
  play: { emoji: "?éæ", msg: "?Ä?ÑÏ£º?∏Ïöî!" },
};
  ...
  const petAction = (action: string) => {
    setPet((prev) => {
      ...
      if (action === "feed") {
        ...
        setShowAction("?çΩÔ∏??†ÎÉ†!");
      } else if (action === "pet") {
        next.petted += 1;
        next.exp += 3;
        setShowAction("?íï Í∞ÑÏ???");
      }
      if (next.exp >= 50) {
        next.level += 1;
        next.exp = 0;
      ...
      <div className="rounded-2xl border border-app-border bg-gradient-to-br from-indigo-500/5 to-purple-500/5 p-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[9px] text-app-text-muted">?ò¥ ?†Ïûê??Ï§?.. (Î∞§Ïù¥?ºÏÑú)</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="rounded-2xl border border-app-border bg-gradient-to-br from-yellow-500/5 to-orange-500/5 p-3 relative overflow-hidden">
      ...
      <div className="flex gap-1.5">
        <button onClick={() => petAction("feed")} className="flex items-center gap-1 rounded-lg bg-orange-500/10 px-2.5 py-1 text-[9px] text-orange-400 hover:bg-orange-500/20 transition-colors">
          <Fish className="h-3 w-3" /> Î∞•Ï£ºÍ∏?        </button>

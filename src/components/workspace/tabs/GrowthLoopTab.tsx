...
const STATUS_LABELS: Record<string, string> = {
  idle: "?€ê¸?, running: "?¤í–‰ ì¤?, paused: "?¼ì‹œ ì¤‘ì?", completed: "?„ë£Œ", failed: "?¤íŒ¨",
};
...
export function GrowthLoopTab() {
  ...
  async function loadLoops() {
    ...
    } catch (e) { console.warn('Unhandled error in GrowthLoopTab', e) }
  ...
  async function startLoop() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const err = await res.json().catch(() => ({ detail: "?œì‘ ?¤íŒ¨" }));
        setError(err.detail || "?œì‘ ?¤íŒ¨");
      }
    } catch { setError("?¤íŠ¸?Œí¬ ?¤ë¥˜"); }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <div>
          <h2 className="text-sm font-bold text-app-text">?ìœ¨ ?±ì¥ ë£¨í”„</h2>
          <p className="text-[10px] text-app-text-muted">ëª©í‘œë§??¤ì •?˜ë©´ AIê°€ ?ë™?¼ë¡œ Analyze?’Generate?’Send?’Repeat</p>
        </div>
      </div>

      {/* Start Panel */}
      <Panel title="?¯ ???±ì¥ ë£¨í”„" className="border-app-primary/20">
        <div className="space-y-3">
          <textarea
            value={goal}
            onChange={(e) => setGoal(e.target.value)}
            placeholder="?¬ì„±?˜ê³  ?¶ì? ëª©í‘œë¥??…ë ¥?˜ì„¸?? ?? ?Œì› 1000ëª?ë§Œë“¤ê¸? ?¼ì¼ ë°œì†¡ 50ê±??¬ì„±, ?„í™˜??15% ë§Œë“¤ê¸?.."
            rows={3}
            disabled={starting}
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-1.5">
              <label className="text-[11px] text-app-text-muted">ì±„ë„</label>
            ...
            <div className="flex items-center gap-1.5">
              <label className="text-[11px] text-app-text-muted">ê°„ê²©(h)</label>
          ...
          <Button variant="primary" onClick={startLoop} loading={starting} disabled={!goal.trim() || starting} className="w-full">
            <Zap className="h-4 w-4" /> ?ìœ¨ ?±ì¥ ?œì‘?˜ê¸°
          </Button>
        </div>
      </Panel>
      ...
      {activeLoops.length > 0 && (
        <Panel title={`???œì„± ë£¨í”„ (${activeLoops.length})`}>
          <div className="space-y-3">
      ...
      {loading ? (
        <div className="flex items-center gap-2 text-xs text-app-text-muted py-4">
          <Loader2 className="h-4 w-4 animate-spin" /> ë¶ˆëŸ¬?¤ëŠ” ì¤?..
        </div>
      ...
      {!loading && loops.length === 0 && (
        <div className="flex flex-col items-center gap-3 py-16 text-center">
          ...
          <p className="text-sm font-medium text-app-text">?ìœ¨ ?±ì¥ ë£¨í”„</p>
          <p className="text-xs text-app-text-muted max-w-xs">
            ëª©í‘œë§??…ë ¥?˜ë©´ AIê°€ ?ë™?¼ë¡œ<br />
            ì½˜í…ì¸??ì„± ??ë°œì†¡ ??ë¶„ì„ ??ê°œì„ ??ë°˜ë³µ?©ë‹ˆ??
          </p>
        </div>
      )}
    </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex gap-3 mt-2 text-[10px]">
              <span className="text-app-text-muted">
                <span className="font-medium text-app-text">{m.cycles_completed || loop.current_cycle}</span> ?¬ì´??
              </span>
              <span className="text-app-text-muted">
                ?„ë‹¬ <span className="font-medium text-app-text">{m.total_reached?.toLocaleString() || 0}</span>ëª?
              </span>
              <span className="text-app-text-muted">
                ?±ê³µë¥?<span className="font-medium text-app-success">{m.avg_success_rate || 0}%</span>
              </span>
            </div>
            ...
            {latestCycle && (
              <div className="mt-2 rounded-lg border border-app-border/50 bg-app-bg/50 p-2">
                <p className="text-[9px] text-app-text-muted">ìµœê·¼ ?¬ì´??#{latestCycle.cycle_number}</p>
                ...
                <div className="flex gap-2 mt-1 text-[9px] text-app-text-muted">
                  <span>{latestCycle.sent_count}ê±?ë°œì†¡</span>
                  <span>{latestCycle.success_rate}%</span>
                  <span>{latestCycle.engagement_count} ì°¸ì—¬</span>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="flex flex-col gap-1 shrink-0">
            ...
            <button onClick={() => setExpanded(!expanded)}
              className="p-1.5 rounded-lg hover:bg-app-card-hover text-app-text-muted transition-colors" title="?ì„¸">
              <ChevronRight className={cn("h-3.5 w-3.5 transition-transform", expanded && "rotate-90")} />
            </button>
            <button onClick={() => onAction(loop.id, "delete")} disabled={actioning === loop.id}
              className="p-1.5 rounded-lg hover:bg-app-danger/10 text-app-text-muted hover:text-app-danger transition-colors" title="?? œ">
          ...
          {loop.cycles.map((c) => (
            <div key={c.cycle_number} className="rounded-lg border border-app-border/50 bg-app-bg/30 p-2">
              <div className="flex items-center justify-between text-[10px]">
                <span className="font-medium text-app-text">?¬ì´??#{c.cycle_number}</span>
              ...
              {c.analysis && (
                <p className="text-[9px] text-app-text-muted mt-1">?“Š {c.analysis}</p>
              )}

    ...
    >
      ...
      {todayUsage && (
        <div className="flex items-center gap-2 rounded-xl border border-app-primary/20 bg-app-primary/5 px-3 py-2">
          ...
          <span className="text-xs font-semibold text-app-text">?§Îäò ?¨Ïö©??/span>
          <span className="text-xs text-app-text-muted">
            ?îÏ≤≠ {todayUsage.total_requests}??¬∑ ?†ÌÅ∞ {todayUsage.total_tokens.toLocaleString()} ¬∑ ?¨Î†à??{todayUsage.total_credits}
          </span>
        </div>
      )}
      ...
      <Panel title="Í∏∞Îä•Î≥??¨Ïö©??>
        <div className="space-y-3">
          {usage?.features && Object.keys(usage.features).length > 0 ? (
              ...
              return (
                <div key={key} className="rounded-xl border border-app-border bg-app-bg p-3">
                  ...
                  <div className="flex items-center gap-4 text-[11px] text-app-text-muted">
                    <span>?†ÌÅ∞: {data.tokens.toLocaleString()}</span>
                    <span>?¨Î†à?? {data.credits}</span>
                    {limit && (
                      <span className={cn(
                        limit.max_requests_per_day > 0 ? "text-app-info" : "text-app-text-subtle"
                      )}>
                        ?ºÏùº ?úÎèÑ: {limit.max_requests_per_day > 0 ? `${limit.max_requests_per_day}?? : "Î¨¥Ï†ú??}
                      </span>
                    )}
                  </div>
            ...
            <p className="text-xs text-app-text-muted text-center py-4">?¨Ïö©???∞Ïù¥?∞Í? ?ÜÏäµ?àÎã§</p>
          )}
        </div>
      </Panel>
      ...
      {/* Plan Limits */}
      {limits.length > 0 && (
        <Panel title="?åÎûú ?úÌïú">
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-app-border">
                  <th className="text-left py-2 px-2 text-app-text-muted font-medium">Í∏∞Îä•</th>
                  <th className="text-right py-2 px-2 text-app-text-muted font-medium">?ºÏùº ?îÏ≤≠</th>
                  <th className="text-right py-2 px-2 text-app-text-muted font-medium">?ºÏùº ?†ÌÅ∞</th>
                  <th className="text-right py-2 px-2 text-app-text-muted font-medium">???¨Î†à??/th>
                  <th className="text-center py-2 px-2 text-app-text-muted font-medium">?ÅÌÉú</th>
                </tr>
              </thead>
              <tbody>
                {limits.map(limit => (

...
const SUB_TABS = [
  { id: "chat", label: "AI ?€??, shortLabel: "?€??, icon: MessageSquare, desc: "AI ?´ì˜ ë¹„ì„œ?€ ?ìœ ë¡?²Œ ?€?? },
  { id: "reply", label: "AI ?µì¥", shortLabel: "?µì¥", icon: MessageCircle, desc: "?¤ë§ˆ???µì¥ ì¶”ì²œ" },
  { id: "broadcast", label: "AI ë°œì†¡", shortLabel: "ë°œì†¡", icon: Megaphone, desc: "AIê°€ ?‘ì„±??ë°œì†¡ ë©”ì‹œì§€" },
  { id: "contentstudio", label: "ì½˜í…ì¸??¤íŠœ?”ì˜¤", shortLabel: "ì½˜í…ì¸?, icon: Sparkles, desc: "AI ì½˜í…ì¸??ì„± & ?ˆì•½ ë°œì†¡" },
  { id: "operations", label: "AI ë¦¬í¬??, shortLabel: "ë¦¬í¬??, icon: BarChart3, desc: "?´ì˜ ë¦¬í¬??ë°??¸ì‚¬?´íŠ¸" },
  { id: "opscenter", label: "AI ?´ì˜ ?¼í„°", shortLabel: "?´ì˜", icon: Gauge, desc: "?µí•© ?´ì˜ ?„í™©" },
  { id: "usage", label: "AI ?¬ìš©??, shortLabel: "?¬ìš©??, icon: Cpu, desc: "AI ê¸°ëŠ¥ ?¬ìš© ?µê³„" },
  { id: "employee", label: "Employee Mode", shortLabel: "ì§ì›", icon: Users, desc: "ê·¸ë£¹ AI ?¤ì • ë°??ˆì•½ ë©”ì‹œì§€" },
];
      ...
      >
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs text-app-text-muted">? í°</span>
          </div>
      ...
      {showInfoBar && (
        <div className="rounded-xl border border-app-border bg-app-card p-3 space-y-3 animate-scale-in">
          ...
          >
            ...
            {streak > 0 ? `?”¥ ì¶œì„ ì²´í¬??(${streak}?¼ì§¸)` : "?”¥ ì²?ì¶œì„ ì²´í¬??}
          </button>

          <div className="text-xs text-app-text-muted space-y-1">
            <p className="font-medium text-app-text flex items-center gap-1"><Zap className="h-3.5 w-3.5 text-amber-500" /> ? í° ?¬ìš©</p>
            <div className="grid grid-cols-2 gap-1">
              <span>AI ì±„íŒ…/?µì¥: 50? í°</span>
              <span>AI ë°œì†¡/ë¶„ì„: 100? í°</span>
              <span>ì½˜í…ì¸??¤íŠœ?”ì˜¤: 200? í°</span>
              <span>ì¶œì„ 7?? +100? í°</span>
            </div>
          </div>
...
function AiChatRedirect() {
  ...
  useEffect(() => {
    ...
    agentApi.fetchAgents()
      ...
      .catch(() => { toast("error", "Agent ë¡œë“œ ?¤íŒ¨"); })
    ...
    <div className="space-y-3">
      <Link href="/app/chat"
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <h3 className="text-sm font-semibold text-app-text">AI Agent ?€?”ë°©</h3>
            <ChevronRight className="h-3.5 w-3.5 text-app-text-muted group-hover:translate-x-0.5 transition-transform" />
          </div>
          <p className="text-xs text-app-text-muted mt-0.5">Agentë¥?ë§Œë“¤ê³??€?”í•˜ë©??ˆë²¨?…í•˜?¸ìš”</p>
        </div>
      </Link>
      ...
      {loading ? (
        <div className="flex items-center justify-center py-8 text-app-text-muted">
          ...
          <span className="text-xs">Agent ë¶ˆëŸ¬?¤ëŠ” ì¤?..</span>
        </div>
      ) : agents.length === 0 ? (
        <div className="flex flex-col items-center gap-3 rounded-xl border border-app-border bg-app-card py-10 px-4 text-center">
          ...
          <p className="text-xs text-app-text-muted">?„ì§ ?ì„±??Agentê°€ ?†ìŠµ?ˆë‹¤</p>
          <Link href="/app/chat"
        ...
        <>
          <div className="grid grid-cols-3 gap-1.5 sm:gap-2">
            ...
            <div className="rounded-lg border border-app-border bg-app-card px-2 py-2 text-center sm:px-3">
              ...
              <div className="text-[9px] sm:text-[10px] text-app-text-muted">ì´??ˆë²¨</div>
            </div>
            <div className="rounded-lg border border-app-border bg-app-card px-2 py-2 text-center sm:px-3">
              <div className="text-base sm:text-lg font-bold text-app-text">{totalMessages}</div>
              <div className="text-[9px] sm:text-[10px] text-app-text-muted">ì´?ë©”ì‹œì§€</div>
            </div>
          </div>

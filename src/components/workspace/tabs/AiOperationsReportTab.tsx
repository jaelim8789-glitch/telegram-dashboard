  ...
  const generateReport = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError(err.detail || "Î¶¨Ìè¨???ùÏÑ± ?§Ìå®");
      }
    } catch {
    ...
    >
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-4">
        ...
        <div className="lg:col-span-1 space-y-3">
          <Panel title="??Î¶¨Ìè¨???ùÏÑ±" className="shrink-0">
            <div className="space-y-3">
              <div>
                <label className="text-[11px] font-medium text-app-text-muted">Î¶¨Ìè¨???†Ìòï</label>
                <select value={reportType} onChange={e => { setReportType(e.target.value); setDays(e.target.value === "daily" ? 1 : e.target.value === "weekly" ? 7 : 30); }}
                  ...
                  <option value="daily">?ºÍ∞Ñ Î¶¨Ìè¨??/option>
                  <option value="weekly">Ï£ºÍ∞Ñ Î¶¨Ìè¨??/option>
                  <option value="custom">ÎßûÏ∂§ Î¶¨Ìè¨??/option>
                </select>
              </div>
              {reportType === "custom" && (
                <div>
                  <label className="text-[11px] font-medium text-app-text-muted">Î∂ÑÏÑù Í∏∞Í∞Ñ (??</label>
              ...
              <button onClick={generateReport} disabled={loading}
                ...
                {loading ? "?ùÏÑ± Ï§?.." : "AI Î¶¨Ìè¨???ùÏÑ±"}
              </button>
              {error && <p className="text-xs text-app-danger">{error}</p>}
            </div>
          </Panel>

          <Panel title="?¥Ï†Ñ Î¶¨Ìè¨?? className="flex-1">
            <div className="space-y-1">
              {reports.length === 0 ? (
                <p className="text-xs text-app-text-muted text-center py-4">?ùÏÑ±??Î¶¨Ìè¨?∏Í? ?ÜÏäµ?àÎã§</p>
              ) : (
                reports.map(r => (
                  <button key={r.id} onClick={() => loadReport(r.id)}
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="capitalize">{r.report_type} Î¶¨Ìè¨??/span>
                    </div>
          ...
          {currentReport ? (
            <div className="space-y-4">
              ...
              {(currentReport.sections?.length ?? 0) > 0 && (
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  {currentReport.sections!.map((section: ReportSection, i: number) => (
                    <Panel key={section.title || `section-${i}`} title={section.title || `?πÏÖò ${i + 1}`}>
              ...
              {(currentReport.insights?.length ?? 0) > 0 && (
                <Panel title={<div className="flex items-center gap-1.5"><Lightbulb className="h-4 w-4 text-app-warning" /> ?∏ÏÇ¨?¥Ìä∏</div>}>
              ...
              {(currentReport.recommendations?.length ?? 0) > 0 && (
                <Panel title={<div className="flex items-center gap-1.5"><TrendingUp className="h-4 w-4 text-app-primary" /> Í∞úÏÑ† Ï∂îÏ≤ú</div>}>
                  <div className="space-y-2">
            ...
            <div className="flex flex-col items-center justify-center h-96 text-center">
              ...
              <p className="text-sm font-medium text-app-text">AI ?¥ÏòÅ Î¶¨Ìè¨??/p>
              <p className="text-xs text-app-text-muted mt-1">?¥ÏòÅ ?∞Ïù¥?∞Î? Î∂ÑÏÑù???∏ÏÇ¨?¥Ìä∏ Î¶¨Ìè¨?∏Î? ?úÍ≥µ?©Îãà??/p>
            </div>
          )}
        </div>
      </div>

...
const STATUS_TONE: Record<BroadcastStatus, { tone: "neutral" | "success" | "warning" | "danger" | "info"; label: string }> = {
  pending: { tone: "neutral", label: "?ÄÍ∏?Ï§? },
  sending: { tone: "info", label: "Î∞úÏÜ° Ï§? },
  sent: { tone: "success", label: "?ÑÎ£å" },
  failed: { tone: "danger", label: "?§Ìå®" },
  cancelled: { tone: "warning", label: "Ï∑®ÏÜå?? },
};
...
// ?Ä?Ä Main Dashboard ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
...
export function DashboardTab() {
  ...
  const WIDGET_LABELS: Record<string, string> = {
    dailyDigest: "?§Îäò???úÎèô", realtimeMetrics: "?§ÏãúÍ∞?Î©îÌä∏Î¶?, usageChart: "?¨Ïö©??Ï∞®Ìä∏",
    usageProgress: "?¨Ïö©???úÎèÑ", attention: "?¥ÏòÅ Ï£ºÏùò ?¨Ìï≠", recentFailures: "ÏµúÍ∑º Î∞úÏÜ° ?§Ìå®",
    middlePanels: "?àÏïΩ/Î∞òÎ≥µ/Í±¥Í∞ï ?®ÎÑê", recentActivity: "ÏµúÍ∑º ?úÎèô", accountOverview: "Í≥ÑÏ†ï ?ÑÌô©",
    failureIntelligence: "?§Ìå® Î∂ÑÏÑù", healthTrend: "Í≥ÑÏ†ï Í±¥Í∞ï ?∏Î†å??, healthScore: "Í≥ÑÏ†ï Í±¥Í∞ï ?êÏàò",
    banPredictor: "Í≥ÑÏ†ï Ï∞®Îã® ?àÏ∏°",
  };

  // ?Ä?Ä Widget visibility ?Ä?Ä
  ...
  // ?Ä?Ä Mobile widget simplification ?Ä?Ä
  ...
  const DEFAULT_PROFILE_NAME = "Í∏∞Î≥∏";
  ...
  // ?Ä?Ä Widget order (drag-reorderable) ?Ä?Ä
  ...
  const priorityQueue = useMemo(() => {
    ...
    if (healthCritical.length > 0) {
      items.push({
        id: "auth",
        title: `?∏Ï¶ù/Ï∞®Îã® Í≥ÑÏ†ï ${healthCritical.length}Í∞?,
        detail: "Î∞úÏÜ° Ï§ëÎã® ?ÑÏóê ?¨Ïù∏Ï¶??êÎäî ?ÅÌÉú ?êÍ????ÑÏöî?©Îãà??",
        cta: "Í≥ÑÏ†ï ?ïÏù∏",
        tone: "danger",
    ...
    if (recentFailures.length > 0) {
      items.push({
        id: "failures",
        title: `ÏµúÍ∑º ?§Ìå® Î∞úÏÜ° ${recentFailures.length}Í±?,
        detail: recentFailures[0]?.message || "?§Ìå® ?êÏù∏???ïÏù∏?òÍ≥† ?¨Ïãú?ÑÌïò?∏Ïöî.",
        cta: "Î°úÍ∑∏ ?¥Í∏∞",
        tone: "warning",
        action: () => setTab("log"),
      });
    ...
    if ((summary?.success_rate ?? 100) < 85) {
      items.push({
        id: "performance",
        title: `?±Í≥µÎ•?${summary?.success_rate?.toFixed(1) ?? "0.0"}%`,
        detail: "ÏµúÍ∑º ?±Í≥ºÍ∞Ä ??äµ?àÎã§. ?úÍ∞Ñ?Ä?Ä Í≥ÑÏ†ïÎ≥??§Ìå® ?êÏù∏??Î∞îÎ°ú ?ïÏù∏?òÏÑ∏??",
        cta: "Î∂ÑÏÑù Î≥¥Í∏∞",
        tone: "warning",
    ...
    if (memoryTopPosts.length > 0) {
      items.push({
        id: "reuse",
        title: "Í≥†ÏÑ±Í≥?Î¨∏Íµ¨ ?¨Ìôú??Í∞Ä??,
        detail: `Top Í∏Ä ${memoryTopPosts[0].success_rate}% ?±Í≥ºÎ•?Ï∞∏Í≥†???¨Ïûë?±Ìï† ???àÏäµ?àÎã§.`,
        cta: "AIÎ°??¨Ïûë??,
        tone: "success",
    ...
    if (upcoming.length === 0 && activeAccounts.length > 0) {
      items.push({
        id: "schedule",
        title: "?§Ïùå Î∞úÏÜ° ?àÏïΩ ?ÜÏùå",
        detail: "Î™®Î∞î?ºÏóê?úÎäî ÎØ∏Î¶¨ 1Í±??àÏïΩ?¥ÎëêÎ©??¥ÏòÅ ?ÑÎùΩ??Ï§ÑÏùº ???àÏäµ?àÎã§.",
        cta: "Î∞úÏÜ° Ï§ÄÎπ?,
        tone: "info",
        action: () => setTab("send"),
      });
  ...
  if (accountsLoading && accounts.length === 0) {
    return (
      <div className="space-y-4" aria-busy="true" aria-label="?Ä?úÎ≥¥??Î°úÎî© Ï§?>
    ...
    return (
      <Panel title="?úÏä§???ÅÌÉú">
        <div className="flex flex-col items-center justify-center py-12 text-center" role="alert">
          ...
          <p className="text-sm font-semibold text-app-danger">?úÎ≤Ñ???∞Í≤∞?????ÜÏäµ?àÎã§</p>
          ...
          <button onClick={loadAll}
            ...
            <RefreshCw className="h-3.5 w-3.5" aria-hidden="true" /> ?§Ïãú ?úÎèÑ
          </button>
        </div>
      </Panel>
    );
  ...
  return (
    <div className="flex flex-col gap-5 pb-8">
      {/* ?Ä?Ä Header Section ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä */}
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-base font-bold text-app-text">?¥ÏòÅ ?Ä?úÎ≥¥??/h1>
            {hasAnyAttention && (
          ...
          <p className="text-xs text-app-text-muted">?§ÏãúÍ∞??¥ÏòÅ ?ÑÌô©</p>
        </div>
        <div className="flex items-center gap-2">
          {accounts.length === 0 && (
            <button onClick={() => setTab("register")}
              ...
              <Plus className="h-3.5 w-3.5" aria-hidden="true" /> Í≥ÑÏ†ï Ï∂îÍ?
            </button>
          )}
          <div className="relative" ref={exportDropdownRef}>
            <button onClick={() => setShowExportDropdown(!showExportDropdown)} aria-label="?¥Î≥¥?¥Í∏∞"
              ...
              <Download className="h-3.5 w-3.5" aria-hidden="true" /> ?¥Î≥¥?¥Í∏∞
            </button>
            {showExportDropdown && (
          ...
          <button onClick={loadAll} aria-label="?àÎ°úÍ≥†Ïπ®"
            className="flex items-center gap-1 rounded-lg px-2.5 py-1.5 text-xs text-app-text-muted hover:text-app-text hover:bg-app-card-hover transition-colors focus-ring">
      ...
      {/* ?Ä?Ä Daily Digest ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä */}
      ...
      {/* ?Ä?Ä Onboarding Wizard (0 accounts) ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä */}
      {accounts.length > 0 && (
      ...
      {accounts.length === 0 && !accountsLoading && (
        <Panel title="?? TeleMon ?úÏûë?òÍ∏∞" className="border-app-primary/20 bg-app-primary-muted/5">
          <div className="space-y-4 py-4">
            <p className="text-sm text-app-text-secondary">
              TeleMon???§Ïã† Í≤ÉÏùÑ ?òÏòÅ?©Îãà?? ?úÏûë?òÎ†§Î©?Telegram Í≥ÑÏ†ï???∞Í≤∞?òÏÑ∏??
            </p>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              {[
                { step: 1, title: "Í≥ÑÏ†ï ?∞Í≤∞", desc: "Telegram Í≥ÑÏ†ï??TeleMon???∞Í≤∞?òÏÑ∏??, icon: Link, action: "Í≥ÑÏ†ï ?±Î°ù?òÍ∏∞", tab: "register" as const },
                { step: 2, title: "Í∑∏Î£π ?†ÌÉù", desc: "Í¥ÄÎ¶¨Ìï† Í∑∏Î£πÍ≥?Ï±ÑÎÑê???†ÌÉù?òÏÑ∏??, icon: Users, action: "Í∑∏Î£π Í¥ÄÎ¶?, tab: "group" as const },
                { step: 3, title: "Ï≤?Î©îÏãúÏßÄ Î∞úÏÜ°", desc: "AI???ÑÏ???Î∞õÏïÑ Ï≤?Î∞úÏÜ°???úÏûë?òÏÑ∏??, icon: Send, action: "Î∞úÏÜ°?òÍ∏∞", tab: "send" as const },
              ].map((item) => (
            ...
            <div className="flex items-center gap-2 text-xs text-app-text-muted border-t border-app-border pt-3">
              ...
              ?ÑÏ????ÑÏöî?òÏãúÎ©?{process.env.NEXT_PUBLIC_SUPPORT_EMAIL || "support@telemon.io"} ?ºÎ°ú Î¨∏Ïùò?òÏÑ∏??
            </div>
          </div>
        </Panel>
      )}
      ...
      <section className="rounded-2xl border border-app-border bg-app-card p-4">
        <div className="flex items-center justify-between gap-2">
          <div>
            ...
            <p className="mt-1 text-[11px] text-app-text-muted">ÏßÄÍ∏?Î∞îÎ°ú Ï≤òÎ¶¨?¥Ïïº ???ëÏóÖÎß?Ï∂îÎ†§??Î≥¥Ïó¨Ï§çÎãà??</p>
          </div>
          <button
            type="button"
        ...
        <div className="mt-4 space-y-2">
          {priorityQueue.length === 0 ? (
            <div className="rounded-xl border border-app-border bg-app-bg px-3 py-3 text-xs text-app-text-muted">
              Ï≤òÎ¶¨ ?∞ÏÑ†?úÏúÑÍ∞Ä ?íÏ? ?¥ÏäàÍ∞Ä ?ÜÏäµ?àÎã§. ÏßÄÍ∏àÏ? Î∞úÏÜ° ?àÏïΩ?¥ÎÇò AI Î¨∏Íµ¨ Í∞úÏÑ†??ÏßÑÌñâ?òÍ∏∞ Ï¢ãÏ? ?ÅÌÉú?ÖÎãà??
            </div>
          ) : (
      ...
      <section className="rounded-2xl border border-app-border bg-app-card p-4">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <p className="mt-1 text-[11px] text-app-text-muted">
              AIÍ∞Ä Ï∞∏Í≥†?òÎäî ?¥Î≤à?? ÏßÄ?úÏ£º, ?ëÎÖÑ ?±Í≥º?Ä Î∞òÏùë Ï¢ãÏ? Í∏Ä?ÖÎãà??
            </p>
          </div>
          <div className="text-[11px] text-app-text-subtle">
            {telemonMemory?.generated_at ? `${formatRelativeTime(telemonMemory.generated_at)} ?ÖÎç∞?¥Ìä∏` : "?∞Ïù¥??Ï§ÄÎπ?Ï§?}
          </div>
        </div>

        <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-3">
          {[
            { label: "?¥Î≤à??, value: memoryPeriods?.this_month },
            { label: "ÏßÄ?úÏ£º", value: memoryPeriods?.last_week },
            { label: "?ëÎÖÑ", value: memoryPeriods?.last_year },
          ].map((item) => (
            <div key={item.label} className="rounded-xl border border-app-border bg-app-bg p-3">
              ...
              <div className="text-[11px] text-app-text-subtle">Î∞úÏÜ° ?úÎèÑ</div>
              <div className="mt-2 flex items-center justify-between text-[11px]">
                <span className="text-app-text-muted">?±Í≥µ</span>
                <span className="font-medium text-app-text">{item.value?.successful ?? 0}</span>
              </div>
              <div className="mt-1 flex items-center justify-between text-[11px]">
                <span className="text-app-text-muted">?±Í≥µÎ•?/span>
        ...
        <div className="mt-4 rounded-xl border border-app-border bg-app-bg p-3">
          <div className="flex items-center gap-2 text-xs font-semibold text-app-text">
            ...
            Î∞òÏùë Ï¢ãÏïò??Í∏Ä TOP 3
          </div>
          {memoryTopPosts.length === 0 ? (
            <p className="mt-3 text-xs text-app-text-muted">?ÑÏßÅ Ï∞∏Í≥†??Í≥†ÏÑ±Í≥?Î∞úÏÜ° ?∞Ïù¥?∞Í? Î∂ÄÏ°±Ìï©?àÎã§. ÏµúÍ∑º 180?????úÎèÑ 3Í±??¥ÏÉÅ??Î∏åÎ°ú?úÏ∫ê?§Ìä∏Î∂Ä??Î©îÎ™®Î¶¨Ïóê ?ìÏûÖ?àÎã§.</p>
          ) : (
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 text-[11px] text-app-text-muted">
                        ...
                        <span>?úÎèÑ {post.attempted}Í±?/span>
                        <span>?±Í≥µ {post.successful}Í±?/span>
                      </div>
                    ...
                    <div className="shrink-0 text-right">
                      ...
                      <div className="text-[10px] text-app-text-subtle">?±Í≥µÎ•?/div>
                    </div>
                  </div>
                </div>
              ))}
      ...
      {/* ?Ä?Ä Real-time Metrics Row ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä */}
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-5">
          <div className="rounded-xl border border-app-border bg-gradient-to-br from-app-card to-app-bg p-3">
            ...
            <div className="text-[10px] text-app-text-subtle">
              {failedCount > 0
                ? <span className="text-app-danger">{((failedCount / Math.max(sentCount + failedCount, 1)) * 100).toFixed(1)}% ?§Ìå®??/span>
                : "?§Ìå® ?ÜÏùå"}
            </div>
          </div>
          <div className="rounded-xl border border-app-border bg-gradient-to-br from-app-card to-app-bg p-3">
            ...
            <div className="text-[10px] text-app-text-subtle">
              {upcoming.length > 0 ? `${upcoming.length}Í±??àÏïΩ?? : "?ÄÍ∏?Ï§??ÜÏùå"}
            </div>
          </div>
          <div className="rounded-xl border border-app-border bg-gradient-to-br from-app-card to-app-bg p-3">
            <div className="flex items-center gap-1.5 text-[11px] font-medium text-app-text-muted">
            ...
            <div className="text-[10px] text-app-text-subtle">
              {summary ? `${summary.successful}/${summary.total_attempted}Í±? : "?∞Ïù¥???ÜÏùå"}
            </div>
          </div>
          <div className="rounded-xl border border-app-border bg-gradient-to-br from-app-card to-app-bg p-3">
            <div className="flex items-center gap-1.5 text-[11px] font-medium text-app-text-muted">
            ...
            <div className="text-[10px] text-app-text-subtle">
              {healthCritical.length > 0 ? `${healthCritical.length}Í∞?Ï£ºÏùò ?ÑÏöî` : "Î™®Îì† Í≥ÑÏ†ï ?ïÏÉÅ"}
            </div>
          </div>
          <div className="rounded-xl border border-app-border bg-gradient-to-br from-app-card to-app-bg p-3">
            <div className="flex items-center gap-1.5 text-[11px] font-medium text-app-text-muted">
            ...
            <div className="text-[10px] text-app-text-subtle">
              {erroredRecurring.length > 0
                ? `${erroredRecurring.length}Í∞??§Î•ò`
                : `${recurring.filter(r => getRecurringState(r) === "active").length}Í∞??úÏÑ±`}
            </div>
          </div>
        </div>
      )}
      ...
      {/* ?Ä?Ä Account Health Score ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä */}
      ...
      {/* ?Ä?Ä Operational Summary Hierarchy ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä */}
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
        <div className="rounded-xl border border-app-border bg-app-card p-3">
          ...
          {inactiveAccounts.length > 0 && (
            <div className="text-[10px] text-app-text-subtle">{inactiveAccounts.length}Í∞?ÎπÑÌôú??/div>
          )}
        </div>
        <div className="rounded-xl border border-app-border bg-app-card p-3">
          <div className="flex items-center gap-1.5 text-[11px] font-medium text-app-text-muted">
          ...
          {healthCritical.length > 0 && <div className="text-[10px] text-app-danger">{healthCritical.length}Í∞??∏Ï¶ù/Ï∞®Îã®</div>}
        </div>
        <div className="rounded-xl border border-app-border bg-app-card p-3">
          <div className="flex items-center gap-1.5 text-[11px] font-medium text-app-text-muted">
          ...
          <div className="text-[10px] text-app-text-subtle">{failedCount > 0 ? `${formatCompact(failedCount)}Í±??§Ìå®` : "?§Ìå® ?ÜÏùå"}</div>
        </div>
        <div className="rounded-xl border border-app-border bg-app-card p-3">
          <div className="flex items-center gap-1.5 text-[11px] font-medium text-app-text-muted">
          ...
          <div className="mt-1 text-lg font-bold tabular-nums text-app-text">{recurring.length}</div>
          <div className="text-[10px] text-app-text-subtle">
            {erroredRecurring.length > 0 ? `${erroredRecurring.length}Í∞??§Î•ò` :
             pausedRecurring.length > 0 ? `${pausedRecurring.length}Í∞??ºÏãú ?ïÏ?` :
             "?ïÏÉÅ"}
          </div>
        </div>
      </div>
      ...
      {/* ?Ä?Ä Operational Attention Queue ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä */}
      {widgetVisibility.attention && hasAnyAttention && (
        <div className="rounded-2xl border border-app-border overflow-hidden">
          <div className="flex items-center gap-2 border-b border-app-border bg-app-card px-4 py-2">
            ...
            <span className="text-xs font-semibold text-app-text">?¥ÏòÅ Ï£ºÏùò ?¨Ìï≠</span>
            <span className="ml-auto text-[11px] text-app-text-muted">{totalAttention}Í±?/span>
          </div>
          <div className="divide-y divide-app-border">
            {healthCritical.filter(h => h.status === "unauthorized").map(h => {
              ...
              return (
                <AttentionItem key={"unauth-" + h.accountId}
                  ...
                  detail="?∏Ï¶ù ?ÑÏöî ???∏ÏÖò??ÎßåÎ£å?òÏóàÍ±∞ÎÇò ?∏Ï¶ù?òÏ? ?äÏïò?µÎãà??
                  actionLabel="?¨Ïù∏Ï¶?
                  tone="warning"
                  action={() => setTab("register")}
                />
            ...
            {healthCritical.filter(h => h.status === "banned").map(h => {
              ...
              return (
                <AttentionItem key={"banned-" + h.accountId}
                  ...
                  label={acct?.name?.trim() || h.phone}
                  detail="Telegram Í≥ÑÏ†ï??Ï∞®Îã®?òÏóà?µÎãà??
                  actionLabel="Í≥ÑÏ†ï Í¥ÄÎ¶?
                  tone="danger"
                  action={() => setTab("register")}
                />
              );
            ...
            {healthWarning.filter(h => h.status === "rate_limited").map(h => {
              ...
              return (
                <AttentionItem key={"ratelimit-" + h.accountId}
                  ...
                  label={acct?.name?.trim() || h.phone}
                  detail="?çÎèÑ ?úÌïú ???àÎ¨¥ ÎßéÏ? ?îÏ≤≠?ºÎ°ú ?ºÏãú ?úÌïú??
                  actionLabel="Î°úÍ∑∏ Î≥¥Í∏∞"
                  tone="warning"
                  action={() => setTab("log")}
                />
              );
            ...
            {errors.health && bannedAccounts.length > 0 && (
              <AttentionItem key="banned-fallback"
                icon={<Ban className="h-3.5 w-3.5 text-app-danger" />}
                label={`Ï∞®Îã®??Í≥ÑÏ†ï ${bannedAccounts.length}Í∞?}
            ...
            {erroredRecurring.length > 0 && (
              <AttentionItem key="recurring-error"
                icon={<AlertTriangle className="h-3.5 w-3.5 text-app-danger" />}
                label={`Î∞òÎ≥µ ?§Ï?Ï§??§Î•ò ${erroredRecurring.length}Í∞?}
            ...
            {pausedRecurring.length > 0 && (
              <AttentionItem key="recurring-paused"
                ...
                label={`?ºÏãú ?ïÏ???Î∞òÎ≥µ ${pausedRecurring.length}Í∞?}
            ...
            {nonRetryableFailures.length > 0 && (
              <AttentionItem key="nonretryable-failures"
                ...
                label={`?¨Ïãú??Î∂àÍ? ?§Ìå® ${nonRetryableFailures.length}Í±?}
      ...
      {!hasAnyAttention && accounts.length > 0 && (
        <div className="flex items-center gap-2.5 rounded-2xl border border-app-success/20 bg-app-success-muted/20 px-4 py-3">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-xs font-medium text-app-success">Î™®Îì† ?úÏä§???ïÏÉÅ</p>
            <p className="text-[11px] text-app-text-muted">{activeAccounts.length}Í∞?Í≥ÑÏ†ï ?úÏÑ± ¬∑ {recurring.length}Í∞?Î∞òÎ≥µ ?§Ï?Ï§?/p>
          </div>
        </div>
      )}
      </div>
      ...
      {/* ?Ä?Ä Recent Failures with retryable classification ?Ä?Ä */}
      {widgetVisibility.recentFailures && recentFailures.length > 0 && (
        <div className="rounded-2xl border border-app-border overflow-hidden">
          <div className="flex items-center gap-2 border-b border-app-border bg-app-card px-4 py-2">
            <AlertTriangle className="h-3.5 w-3.5 text-app-danger" aria-hidden="true" />
            <span className="text-xs font-medium text-app-text">ÏµúÍ∑º Î∞úÏÜ° ?§Ìå®</span>
            <span className="ml-auto text-[11px] text-app-text-muted">{recentFailures.length}Í±?/span>
          </div>
          <div className="divide-y divide-app-border">
            {recentFailures.map(f => {
              ...
              const displayError = failureSummary || f.errorMessage || "?????ÜÎäî ?§Î•ò";
                ...
                <div key={f.id} className="flex items-center justify-between px-4 py-2">
                  <div className="min-w-0 flex-1 pr-2">
                    <div className="flex items-center gap-1.5">
                      ...
                      {retryable === "retryable" && <Badge tone="warning" className="shrink-0 text-[9px] px-1 py-0">?¨Ïãú??Í∞Ä??/Badge>}
                      {retryable === "not_retryable" && <Badge tone="danger" className="shrink-0 text-[9px] px-1 py-0">?¨Ïãú??Î∂àÍ?</Badge>}
                      {retryable === "conditional" && <Badge tone="info" className="shrink-0 text-[9px] px-1 py-0">Ï°∞Í±¥Î∂Ä</Badge>}
                    </div>
                  ...
                  <button onClick={() => setTab(recoveryTab)}
                    ...
                    {recoveryTab === "register" ? "Í≥ÑÏ†ï Í¥ÄÎ¶? : "Î°úÍ∑∏ Î≥¥Í∏∞"} <ArrowRight className="h-3 w-3" />
                  </button>
                </div>
              );
            })}
          ...
          <button onClick={() => setTab("log")}
            ...
            ?ÑÏ≤¥ Î°úÍ∑∏ Î≥¥Í∏∞ <ArrowRight className="h-3 w-3" />
          </button>
        </div>
      )}
      </div>
      ...
      {/* ?Ä?Ä Usage Chart & Usage Progress (side by side) ?Ä?Ä */}
      ...
      {/* ?Ä?Ä Widget Customization & Profiles ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä */}
        ...
        <div className="relative">
          ...
          {showWidgetSettings && (
            <div className="absolute left-0 top-full z-20 mt-1 w-64 rounded-xl border border-app-border bg-app-surface p-2 shadow-xl">
              <p className="mb-1.5 px-2 text-[10px] font-semibold uppercase tracking-wider text-app-text-muted">?ÑÏ†Ø ?úÏÑú Î∞??úÏãú</p>
              {widgetKeys.map((w, i) => (
                <div
                  key={w.key}
                  draggable
          ...
          {showProfilePanel && (
            <div className="absolute left-0 top-full z-20 mt-1 w-56 rounded-xl border border-app-border bg-app-surface p-2 shadow-xl">
              ...
              <div className="mt-2 border-t border-app-border pt-2">
                ...
                >
                  <Plus className="h-3 w-3" /> ???ÑÎ°ú?ÑÎ°ú ?Ä??
                </button>
              </div>
              {profilePrompt !== null && (
                <div className="mt-2 border-t border-app-border pt-2">
                  <form onSubmit={(e) => { e.preventDefault(); if (profilePrompt.trim()) { saveCurrentProfile(profilePrompt.trim()); setProfilePrompt(null); setShowProfilePanel(false); } }}>
                    ...
                    <div className="mt-1.5 flex justify-end gap-1">
                      <button type="button" onClick={() => setProfilePrompt(null)}
                        className="rounded-lg px-2 py-1 text-[11px] text-app-text-muted hover:bg-app-card-hover transition-colors">Ï∑®ÏÜå</button>
                      <button type="submit"
                        className="rounded-lg bg-app-primary px-2 py-1 text-[11px] text-white hover:bg-app-primary-hover transition-colors">?Ä??/button>
                    </div>
                  </form>
                </div>
              )}
      ...
      {/* ?Ä?Ä Quick Actions ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä */}
      <div className="flex flex-wrap items-center gap-2">
        <button onClick={() => setTab("send")}
          className="flex items-center gap-1.5 rounded-xl bg-app-primary px-3.5 py-2 text-xs font-medium text-white shadow-sm shadow-app-primary/20 hover:bg-app-primary-hover transition-colors focus-ring" aria-label="??Î∞úÏÜ°">
          <Plus className="h-3.5 w-3.5" aria-hidden="true" /> ??Î∞úÏÜ°
        </button>
        <button onClick={() => setTab("groupsearch")}
          ...
          <Users className="h-3.5 w-3.5" aria-hidden="true" /> Í∑∏Î£π Ï∞æÍ∏∞
        </button>
        <button onClick={() => setTab("scheduler")}
          className="flex items-center gap-1.5 rounded-xl border border-app-border bg-app-card px-3.5 py-2 text-xs font-medium text-app-text hover:border-app-border-strong hover:shadow-sm transition-colors focus-ring">
          <Clock className="h-3.5 w-3.5" aria-hidden="true" /> Î∞òÎ≥µ ?§Ï?Ï§ÑÎü¨
        </button>
        <button onClick={() => setTab("deliveryanalytics")}
          className="flex items-center gap-1.5 rounded-xl border border-app-border bg-app-card px-3.5 py-2 text-xs font-medium text-app-text hover:border-app-border-strong hover:shadow-sm transition-colors focus-ring">
          <BarChart3 className="h-3.5 w-3.5" aria-hidden="true" /> ?ÑÎã¨ Î∂ÑÏÑù
        </button>
        <button onClick={() => setTab("log")}
          className="flex items-center gap-1.5 rounded-xl border border-app-border bg-app-card px-3.5 py-2 text-xs font-medium text-app-text hover:border-app-border-strong hover:shadow-sm transition-colors focus-ring">
          <Activity className="h-3.5 w-3.5" aria-hidden="true" /> Î∞úÏÜ° Î°úÍ∑∏
        </button>
        <button onClick={() => setTab("register")}
          ...
          <UserPlus className="h-3.5 w-3.5" aria-hidden="true" /> Í≥ÑÏ†ï Í¥ÄÎ¶?
        </button>
      </div>

      <div key="middlePanels" style={{ order: widgetIdx("middlePanels") }}>
      {/* ?Ä?Ä Middle Section: 3 columns ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä */}
      {widgetVisibility.middlePanels && (
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
          <Panel
            title={<div className="flex items-center gap-2"><Clock className="h-4 w-4 text-app-info" aria-hidden="true" /> ?àÏïΩ??Î∞úÏÜ°</div>}
          ...
          >
            {dataLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-6 text-center">
                ...
                <p className="text-xs text-app-text-muted">?àÏïΩ??Î∞úÏÜ°???ÜÏäµ?àÎã§</p>
              </div>
            ) : (
                  ...
                  <div key={b.id} className="flex items-center justify-between rounded-xl border border-app-border bg-app-bg px-3 py-2 transition-colors hover:border-app-border-strong">
                    <div className="min-w-0 flex-1 pr-2">
                      ...
                      <p className="text-[11px] text-app-text-subtle">
                        ...
                        {" ¬∑ "}{b.recipients.length}Í∞??Ä??
                      </p>
                    </div>
                    <Badge tone="info">?àÏïΩ</Badge>
                  </div>
                ))}
              </div>
            )}
          ...
          <BroadcastListSection
            title={<div className="flex items-center gap-2"><RefreshCw className="h-4 w-4 text-app-primary" aria-hidden="true" /> Î∞òÎ≥µ Î∞úÏÜ°</div>}
            items={recurring}
            accounts={accounts}
            dataLoading={dataLoading}
            emptyIcon={<RefreshCw className="mb-2 h-6 w-6 text-app-text-subtle" aria-hidden="true" />}
            emptyMessage="Î∞òÎ≥µ Î∞úÏÜ° ?ºÏ†ï???ÜÏäµ?àÎã§"
            onViewAll={setTab}
      ...
      {/* ?Ä?Ä Recent Activity ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä */}
      {widgetVisibility.recentActivity && (
        <Panel
          title={<div className="flex items-center gap-2"><Activity className="h-4 w-4 text-app-primary" aria-hidden="true" /> ÏµúÍ∑º ?úÎèô</div>}
          className="w-full"
        >
          {dataLoading && recentLogs.length === 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-8 text-center">
              ...
              <p className="text-sm font-medium text-app-text">?∞Í≤∞??Í≥ÑÏ†ï???ÜÏäµ?àÎã§</p>
              <p className="mt-1 text-xs text-app-text-muted">Í≥ÑÏ†ï ?±Î°ù ??óê????Í≥ÑÏ†ï??Ï∂îÍ??òÏÑ∏??/p>
            </div>
          ) : recentLogs.length === 0 && !dataLoading ? (
            <div className="flex flex-col items-center justify-center py-8 text-center">
              <MessageSquare className="mb-2 h-6 w-6 text-app-text-subtle" aria-hidden="true" />
              <p className="text-sm font-medium text-app-text">?ÑÏßÅ ?úÎèô Í∏∞Î°ù???ÜÏäµ?àÎã§</p>
              <p className="mt-1 text-xs text-app-text-muted">Í≥ÑÏ†ï???∞Í≤∞?òÍ≥† Î©îÏãúÏßÄÎ•?Î∞úÏÜ°?òÎ©¥ ?¨Í∏∞???úÏãú?©Îãà??/p>
            </div>
          ) : (
            <div className="divide-y divide-app-border">
              {recentLogs.map((b) => {
                  ...
                  <div key={b.id} className="flex items-center justify-between py-2.5 text-sm">
                    <div className="min-w-0 flex-1 pr-3">
                      <div className="flex items-center gap-1.5">
                        ...
                        {retryable === "retryable" && <Badge tone="warning" className="shrink-0 text-[9px] px-1 py-0">?¨Ïãú??Í∞Ä??/Badge>}
                        {retryable === "not_retryable" && <Badge tone="danger" className="shrink-0 text-[9px] px-1 py-0">?¨Ïãú??Î∂àÍ?</Badge>}
                        {isRecur && <Badge tone="info" className="shrink-0 text-[9px] px-1 py-0">Î∞òÎ≥µ</Badge>}
                      </div>
                    ...
                    <button onClick={() => setTab(isFailed ? "log" : isRecur ? "scheduler" : "send")}
                      ...
                      aria-label={isFailed ? "Î°úÍ∑∏ Î≥¥Í∏∞" : isRecur ? "?§Ï?Ï§ÑÎü¨ Î≥¥Í∏∞" : "Î∞úÏÜ° Î≥¥Í∏∞"}>
                      {isFailed ? "Î°úÍ∑∏ Î≥¥Í∏∞" : isRecur ? "?§Ï?Ï§ÑÎü¨" : "Î∞úÏÜ° Î≥¥Í∏∞"}
                    </button>
                  </div>
                );
              })}
          ...
          {errors.logs && logs.length > 0 && (
            <p className="mt-2 text-[11px] text-app-warning">Î°úÍ∑∏ ?∞Ïù¥?∞Î? Î∂àÎü¨?§Îäî Ï§??ºÎ? ?§Î•òÍ∞Ä Î∞úÏÉù?àÏäµ?àÎã§</p>
          )}
        </Panel>
      )}
      </div>
      ...
      {/* ?Ä?Ä Account Overview Table ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä */}
      {widgetVisibility.accountOverview && (
        <Panel
          title={<div className="flex items-center gap-2"><Users className="h-4 w-4 text-app-primary" aria-hidden="true" /> Í≥ÑÏ†ï ?ÑÌô©</div>}
          description="?∞Í≤∞??Î™®Îì† Telegram Í≥ÑÏ†ï???ÅÌÉú?Ä Ï£ºÏöî ÏßÄ??
        >
          {accounts.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-8 text-center">
            ...
            <div className="relative">
              <div className="overflow-x-auto -mx-1">
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead>Í≥ÑÏ†ï</TableHead>
                      <TableHead>?ÅÌÉú</TableHead>
                      <TableHead className="hidden sm:table-cell">?êÎèô ?ëÎãµ</TableHead>
                      <TableHead className="text-right">?§Îäò</TableHead>
                      <TableHead className="text-right hidden sm:table-cell">Í∑∏Î£π</TableHead>
                      <TableHead className="hidden md:table-cell">ÏµúÍ∑º ?úÎèô</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {accounts.map((a) => {
                        ...
                        <TableRow key={a.id} className={hasHealthIssue ? "bg-app-danger-muted/5" : undefined}>
                          ...
                          <TableCell>
                            <span className={cn(
                              ...
                              {a.status === "active" ? (hasHealthIssue ? "Ï£ºÏùò" : "?úÏÑ±") : a.status === "inactive" ? "ÎπÑÌôú?? : "Ï∞®Îã®"}
                            </span>
                          </TableCell>
                          <TableCell className="hidden sm:table-cell">
                            {a.autoReplyEnabled
                              ? <span className="text-app-success text-xs font-medium">ÏºúÏßê</span>
                              : <span className="text-app-text-subtle text-xs">Í∫ºÏßê</span>}
                          </TableCell>
      ...
      {/* ?Ä?Ä Health Trend ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä */}
      {widgetVisibility.healthTrend && healthItems.length > 0 && (
        <Panel
          title={<div className="flex items-center gap-2"><HeartPulse className="h-4 w-4 text-app-primary" aria-hidden="true" /> Í≥ÑÏ†ï Í±¥Í∞ï ?∏Î†å??/div>}
          description="Í∞?Í≥ÑÏ†ï??ÏµúÍ∑º Î∞úÏÜ° ?±Í≥µÎ•?Î∞?Í±¥Í∞ï ?ÅÌÉú"
        >
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
            {healthItems.map((h) => {
              ...
              const statusLabel: Record<string, { label: string; tone: string }> = {
                healthy: { label: "?ïÏÉÅ", tone: "text-app-success" },
                unauthorized: { label: "?∏Ï¶ù ?ÑÏöî", tone: "text-app-warning" },
                banned: { label: "Ï∞®Îã®", tone: "text-app-danger" },
                rate_limited: { label: "?úÌïú", tone: "text-app-warning" },
                error: { label: "?§Î•ò", tone: "text-app-danger" },
                not_configured: { label: "ÎØ∏ÏÑ§??, tone: "text-app-text-muted" },
                unknown: { label: "?????ÜÏùå", tone: "text-app-text-muted" },
              };
              ...
              return (
                <div key={h.accountId} className="rounded-xl border border-app-border bg-app-card p-3 transition-colors hover:border-app-border-strong">
                  ...
                  {h.lastActivity && (
                    <p className="mt-1 text-[9px] text-app-text-subtle">ÎßàÏ?Îß??úÎèô: {formatRelativeTime(h.lastActivity)}</p>
                  )}
                </div>
              );
            })}
                ...
                <div key={bp.accountId} className="rounded-xl border border-app-border bg-app-card p-3 transition-colors hover:border-app-border-strong">
                  ...
                  {(bp.riskLevel === "critical" || bp.riskLevel === "high") && bp.estimatedDaysLeft !== undefined && (
                    <p className="mt-0.5 text-[10px] text-app-danger">
                      ?àÏÉÅ {bp.estimatedDaysLeft}????Ï∞®Îã®
                    </p>
                  )}
                </div>
              );
      ...
      {/* ?Ä?Ä Failure Intelligence ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä */}
      {widgetVisibility.failureIntelligence && failureTypes.length > 0 && (
        <Panel
          title={<div className="flex items-center gap-2"><AlertTriangle className="h-4 w-4 text-app-danger" aria-hidden="true" /> ?§Ìå® Î∂ÑÏÑù</div>}
          description="Ï£ºÏöî ?§Ìå® ?†Ìòï Î∞??ÅÌñ•"
        >
            ...
            {failureTypes.map((f) => (
              <div key={f.status} className="rounded-xl border border-app-border bg-app-card p-3 transition-colors hover:border-app-border-strong">
                ...
                <div className="mt-1.5 flex items-center justify-between text-[11px] text-app-text-muted">
                  <span>{f.count}Í±?/span>
                  <span>{f.affected_accounts}Í∞?Í≥ÑÏ†ï</span>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* ?Ä?Ä API errors banner ??non-blocking ?Ä?Ä */}
      {(errors.logs || errors.upcoming || errors.recurring || errors.overview) && (
        <div className="flex flex-wrap items-center gap-2 rounded-xl border border-app-warning/20 bg-app-warning-muted/20 px-3 py-2 text-[11px] text-app-warning">
          ...
          <span>?ºÎ? ?∞Ïù¥?∞Î? Î∂àÎü¨?§Ï? Î™ªÌñà?µÎãà??</span>
          <button onClick={loadAll} className="ml-auto underline hover:no-underline focus-ring">?§Ïãú ?úÎèÑ</button>
        </div>
      )}
      ...
      {errors.health && (
        <div className="flex flex-wrap items-center gap-2 rounded-xl border border-app-warning/20 bg-app-warning-muted/20 px-3 py-2 text-[11px] text-app-warning">
          <AlertTriangle className="h-3.5 w-3.5 shrink-0" aria-hidden="true" />
          <span>Í≥ÑÏ†ï ?ÅÌÉú ?ïÎ≥¥Î•?Î∂àÎü¨?????ÜÏäµ?àÎã§.</span>
          <button onClick={loadHealth} className="ml-auto underline hover:no-underline focus-ring">?§Ïãú ?úÎèÑ</button>
        </div>
      )}
    </div>
  );

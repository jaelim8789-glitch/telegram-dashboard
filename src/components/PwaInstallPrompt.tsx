 ...
 * PWA ?치 ?롬?트 ??beforeinstallprompt ?벤?? 캐치?여

 * "???치?기" 버튼???시?니?? ?용?? 거절?면 7?간 ?시 ??보입?다.
      ...
      <div className="mx-auto max-w-sm rounded-2xl border border-app-border/60 bg-app-card p-4 shadow-2xl">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm font-semibold text-app-text">TeleMon ???치</p>

            <p className="text-xs text-app-text-muted mt-0.5">

              {promptMode === "native"

                ? "?화면에 추??면 ?음 ?행부????빠르??정?으??립?다"

                : "iPhone/iPad?서??공유 버튼???른 ??'???면??추?'??택?주?요"}
        ...
        {promptMode === "native" ? (
          ...
          <div className="mt-3 rounded-xl border border-app-border/70 bg-app-bg px-3 py-2 text-[12px] text-app-text-muted">
            ...
            Safari ?단??<span className="font-semibold text-app-text">공유</span> 버튼 ??<span className="font-semibold text-app-text">???면??추?</span>

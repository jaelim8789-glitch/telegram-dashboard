  ...
  const fetchLoops = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError('ë£¨í ëª©ë¡??ë¶ë¬?¤ë???¤í¨?ìµ?ë¤.');
      }
    } catch {
      setError('ë£¨í ëª©ë¡??ë¶ë¬?¤ë???¤í¨?ìµ?ë¤.');
    } finally {
  ...
  return (
    <div className="p-6 bg-white rounded-xl shadow-sm border border-gray-200">
      ...
      <div className="mb-6 p-4 bg-indigo-50 rounded-lg">
        <h3 className="font-medium text-indigo-900 mb-2">?ë¡??ë£¨í ?ì±</h3>
        <div className="flex gap-2">
          <input
            type="text"
            value={newGoal}
            onChange={(e) => setNewGoal(e.target.value)}
            placeholder="?? '?ì 1000ëª?ë§ë¤ê¸?"
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(loop.status)}`}>
                      {loop.status === 'running' ? '?¤í ì¤? : 
                       loop.status === 'completed' ? '?ë£?? : 
                       loop.status === 'failed' ? '?¤í¨' : 
                       loop.status === 'paused' ? '?¼ì ?ì?' : '?ê¸?ì¤?}
                    </span>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-3">
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span className="text-gray-600">?ë¬: {loop.totalReached}</span>
                    </div>
                    <div className="flex items-center gap-2 text-sm">
                      <Calendar className="h-4 w-4 text-gray-400" />
                      <span className="text-gray-600">?¬ì´?? {loop.currentCycle}</span>
                    </div>
                    <div className="flex items-center gap-2 text-sm">
                      <TrendingUp className="h-4 w-4 text-gray-400" />
                      <span className="text-gray-600">?í: {loop.conversionRate.toFixed(1)}%</span>
                    </div>
                    <div className="flex items-center gap-2 text-sm">
                      <Activity className="h-4 w-4 text-gray-400" />
                      <span className="text-gray-600">ì°¸ì¬: {loop.engagementRate.toFixed(1)}%</span>
                    </div>
                  </div>
                </div>
              ...
              {selectedLoop === loop.id && (
                <div className="mt-4 pt-4 border-t border-gray-200 text-sm text-gray-600">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <p><strong>?ì±??</strong> {new Date(loop.createdAt).toLocaleString()}</p>
                      <p><strong>?ë°?´í¸:</strong> {new Date(loop.updatedAt).toLocaleString()}</p>
                    </div>
                    <div>
                      <p><strong>ëª©í:</strong> {loop.goal}</p>
                      <p><strong>?í:</strong> {loop.status}</p>
                    </div>
                  </div>
                </div>
              )}

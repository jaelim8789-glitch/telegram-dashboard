...
import { Send, Sparkles, Loader2, Mic, MicOff, Bookmark, BookmarkCheck } from "lucide-react";
...
function ChatBubble({ msg, onBookmark }: { msg: Message; onBookmark: (id: string) => void }) {
  const isUser = msg.role === "user";
  return (
    <div className={`flex ${isUser ? "justify-end" : "justify-start"} group`}>
      <div className="relative max-w-[85%]">
        <div className={`rounded-2xl px-4 py-2.5 text-sm ${isUser ? "rounded-br-md" : "rounded-bl-md"}`}
          style={{ backgroundColor: isUser ? "var(--tg-theme-button-color, #5288c1)" : "var(--tg-theme-section-bg-color, #232e3c)", color: isUser ? "var(--tg-theme-button-text-color, #ffffff)" : "var(--tg-theme-text-color, #f5f5f5)" }}>
          {msg.content}
        </div>
        {!isUser && (
...
export const MiniAppChat = memo(function MiniAppChat() {
  const [messages, setMessages] = useState<Message[]>([
    { role: "agent", content: "안녕하세요! TeleMon AI입니다. 무엇을 도와드릴까요?", id: "welcome" },
  ]);
      ...
      <div className="px-4 shrink-0">
        <div className="flex gap-2">
          ...
          <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); handleSend(input); } }}
            placeholder="메시지를 입력하세요..." autoComplete="off" autoCorrect="off" autoCapitalize="sentences" spellCheck="false"

...
export const MiniAppSend = memo(function MiniAppSend({ user }: MiniAppSendProps) {
  ...
  const [accounts, setAccounts] = useState<{ id: string; phone: string; todaySent?: number; healthScore?: number }[]>([]);
  const [selectedAccountIds, setSelectedAccountIds] = useState<string[]>([]);
  const [groups, setGroups] = useState<{ id: string; title: string; participantsCount?: number }[]>([]);
  ...
  useEffect(() => {
    const cleanup = attachSwipeListeners(swipeRef.current);
  ...
  useEffect(() => {
    ...
    api.fetchAccounts().then(list => {
      const active = list.filter((a: any) => a.status === "active").map((a: any) => ({ id: a.id, phone: a.phone, todaySent: a.todaySent ?? 0, healthScore: a.healthScore ?? 85 }));
      setAccounts(active);
  ...
  }, [message, imageFiles, sending, selectedAccountIds, selectedGroupIds, toast, draft]);
  ...
  return (
    <div className="p-4 pb-8 space-y-3" style={{ paddingBottom: "calc(env(safe-area-inset-bottom) + 280px)" }}>
      ...
      <div className="space-y-1">
        ...
        {loadingAccts ? (
          <div className="flex gap-1.5">
            {[1, 2, 3].map(i => <div key={i} className="h-10 w-28 rounded-full animate-pulse" style={{ backgroundColor: "var(--tg-theme-secondary-bg-color)" }} />)}
          </div>
        ) : (
          <div className="flex flex-wrap gap-1.5">
            {accounts.map(a => (
              <button key={a.id} onClick={() => toggleAccount(a.id)}
                className="rounded-full px-3 py-2 text-[11px] font-medium active:scale-95 min-h-[44px] min-w-[44px] flex items-center gap-1.5"
                ...
                <span className={`h-1.5 w-1.5 rounded-full ${a.healthScore && a.healthScore > 70 ? "bg-emerald-400" : a.healthScore && a.healthScore > 40 ? "bg-amber-400" : "bg-red-400"}`} />
                {a.phone}
                {(a as any).todaySent ? <span className="text-[9px] opacity-60">{(a as any).todaySent}회</span> : null}
              </button>
            ))}
          </div>
        )}
        {accounts.length === 0 && !loadingAccts && (
          <div className="flex items-center gap-2 rounded-xl px-3 py-2 text-xs min-h-[44px]" style={{ backgroundColor: "var(--tg-theme-section-bg-color, #232e3c)", color: "var(--tg-theme-hint-color)" }}>
      ...
      {/* Group selection */}
      <div>
        <button onClick={() => setShowGroupPicker(!showGroupPicker)} className="flex items-center justify-between w-full rounded-xl px-4 py-3 text-sm active:scale-[0.98] min-h-[44px]"
          ...
          <div className="mt-1 max-h-40 overflow-y-auto rounded-xl p-2" style={{ backgroundColor: "var(--tg-theme-secondary-bg-color, #232e3c)" }}>
            {groups.length === 0 ? (
              ...
              <button key={g.id} onClick={() => toggleGroup(g.id)}
                className={`flex w-full items-center gap-2 rounded-lg px-3 py-2 text-xs min-h-[44px] ${selectedGroupIds.includes(g.id) ? "font-semibold" : ""}`}
      ...
      <div>
        ...
        <div className="flex gap-2">
          <button onClick={() => handlePickImage(true)}
            className="flex items-center gap-1.5 rounded-xl px-3 py-2.5 text-xs font-medium active:scale-95 min-h-[44px] min-w-[44px]"
            style={{ backgroundColor: "var(--tg-theme-secondary-bg-color, #232e3c)", color: "var(--tg-theme-text-color)" }}>
        ...
        {/* Image preview grid (multi) */}
        {imagePreviewUrls.length > 0 && (
          <div className="mt-2 grid grid-cols-3 gap-2">
            {imagePreviewUrls.map((url, i) => (
              <div key={i} className="relative rounded-xl overflow-hidden aspect-square" style={{ backgroundColor: "var(--tg-theme-secondary-bg-color, #232e3c)" }}>
                <img src={url} alt={`첨부 이미지 ${i + 1}`} className="w-full h-full object-cover" />
                <button onClick={() => handleRemoveImage(i)}
                  className="absolute top-1 right-1 flex h-6 w-6 items-center justify-center rounded-full active:scale-90"
                  style={{ backgroundColor: "rgba(0,0,0,0.6)" }} aria-label="이미지 제거">
                  <X className="h-3 w-3 text-white" />
                </button>
                {imageFiles[i] && imageFiles[i].size > 5 * 1024 * 1024 && (
                  <div className="absolute bottom-0 inset-x-0 flex items-center justify-center gap-1 px-1 py-0.5 text-[9px]"
                    style={{ backgroundColor: "rgba(239,68,68,0.8)", color: "#fff" }}>
                    <AlertCircle className="h-2.5 w-2.5" /> 5MB 초과
                  </div>
                )}
              </div>
            ))}
      ...
      {/* Message textarea with auto-height */}
      <div className="relative" ref={swipeRef}>
        <textarea ref={textareaRef} value={message} onChange={e => { setMessage(e.target.value); draft.onChange(e.target.value); }}
          placeholder="보낼 메시지를 입력하세요..." rows={2} maxLength={MAX_CHARS}
          style={{ backgroundColor: "var(--tg-theme-secondary-bg-color, #232e3c)", color: "var(--tg-theme-text-color)" }}
          className="w-full rounded-xl px-4 py-3 text-sm outline-none resize-none min-h-[44px] max-h-[280px] overflow-y-auto"
          aria-label="발송 메시지" 
      ...
      {selectedAccountIds.length > 0 && !replyMacroLoading && (
        <div className="flex items-center justify-between rounded-xl px-4 py-3 min-h-[44px]"
          ...
          <label className="relative inline-flex h-6 w-11 cursor-pointer items-center min-h-[44px]">
      ...
      <motion.button
        onClick={() => confirm("발송 확인", handleSend, `${selectedAccountIds.length}개 계정 · ${selectedGroupIds.length || "상위 5"}개 그룹${imageFiles.length > 0 ? ` · 이미지 ${imageFiles.length}장` : ""}으로 발송합니다`)}
        disabled={(!message.trim() && imageFiles.length === 0) || sending || selectedAccountIds.length === 0}
        ...
        className="flex w-full items-center justify-center gap-2 rounded-xl px-4 py-4 text-sm font-semibold disabled:opacity-50 active:scale-[0.98] min-h-[44px]"

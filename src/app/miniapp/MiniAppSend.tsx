...
export const MiniAppSend = memo(function MiniAppSend({ user }: MiniAppSendProps) {
  ...
  const handleSend = useCallback(async () => {
      ...
      try {
        if (selectedGroupIds.length > 0) {
          ...
          if (imageFile) {
            ...
            if (groupIds.length === 0) throw new Error("?∞Í≤∞??Í∑∏Î£π???ÜÏäµ?àÎã§");
            const form = new FormData();
            form.append("account_id", accountId);
            form.append("message", message.trim() || "");
    ...
    if (failed === 0) {
      ...
      toast({ type: "success", title: "Î∞úÏÜ° ?ÑÎ£å", message: `${success}Í∞?Í≥ÑÏ†ï Î∞úÏÜ° ?±Í≥µ` });
      setTimeout(() => setSent(false), 3000);
    } else {
      setError(`${success}Í±??±Í≥µ, ${failed}Í±??§Ìå®`);
  ...
  return (
    <div className="p-4 pb-8 space-y-3">
      <div className="flex items-center justify-between">
        <p className="text-sm font-semibold" style={{ color: "var(--tg-theme-text-color)" }}>Î∞úÏÜ°</p>
        <SuccessPredictorBadge text={message} />
      </div>

      {/* Account selection */}
      <div className="space-y-1">
        <label className="text-[10px] font-medium flex items-center gap-1" style={{ color: "var(--tg-theme-hint-color)" }}><Users className="h-3 w-3" /> Í≥ÑÏ†ï ({selectedAccountIds.length}Í∞?</label>
        ...
        {accounts.length === 0 && !loadingAccts && (
          <div className="flex items-center gap-2 rounded-xl px-3 py-2 text-xs" style={{ backgroundColor: "var(--tg-theme-section-bg-color, #232e3c)", color: "var(--tg-theme-hint-color)" }}>
            <Info className="h-3.5 w-3.5" /> ?úÏÑ± Í≥ÑÏ†ï???ÜÏäµ?àÎã§. ?ÑÎ°ú????óê??Í≥ÑÏ†ï???∞Í≤∞?òÏÑ∏??
          </div>
        )}
      </div>
      ...
      <div>
        <button onClick={() => setShowGroupPicker(!showGroupPicker)} className="flex items-center justify-between w-full rounded-xl px-4 py-3 text-sm active:scale-[0.98]"
          ...
          <span>{selectedGroupIds.length > 0 ? `${selectedGroupIds.length}Í∞?Í∑∏Î£π ?†ÌÉù?? : "Í∑∏Î£π ?†ÌÉù (?†ÌÉù?¨Ìï≠)"}</span>
        ...
        {showGroupPicker && (
          <div className="mt-1 max-h-40 overflow-y-auto rounded-xl p-2" style={{ backgroundColor: "var(--tg-theme-secondary-bg-color, #232e3c)" }}>
            {groups.length === 0 ? (
              <p className="text-xs py-3 text-center" style={{ color: "var(--tg-theme-hint-color, #708499)" }}>Í∑∏Î£π???ÜÏäµ?àÎã§</p>
            ) : groups.map(g => (
      ...
      <div>
        <label className="text-[10px] font-medium flex items-center gap-1 mb-1" style={{ color: "var(--tg-theme-hint-color)" }}>
          <Image className="h-3 w-3" /> ?¥Î?ÏßÄ Ï≤®Î? (?†ÌÉù?¨Ìï≠)
        </label>
        <div className="flex gap-2">
          <button onClick={() => handlePickImage(true)}
            ...
            style={{ backgroundColor: "var(--tg-theme-secondary-bg-color, #232e3c)", color: "var(--tg-theme-text-color)" }}>
            <Camera className="h-4 w-4" /> Ï¥¨ÏòÅ
          </button>
          <button onClick={() => handlePickImage(false)}
            className="flex items-center gap-1.5 rounded-xl px-3 py-2.5 text-xs font-medium active:scale-95"
            style={{ backgroundColor: "var(--tg-theme-secondary-bg-color, #232e3c)", color: "var(--tg-theme-text-color)" }}>
            <Image className="h-4 w-4" /> ?®Î≤î
          </button>
        ...
        {imagePreviewUrl && (
          <div className="relative mt-2 rounded-xl overflow-hidden" style={{ backgroundColor: "var(--tg-theme-secondary-bg-color, #232e3c)" }}>
            <img src={imagePreviewUrl} alt="Ï≤®Î? ?¥Î?ÏßÄ" className="w-full max-h-48 object-contain" />
            <button onClick={handleRemoveImage}
              className="absolute top-2 right-2 flex h-7 w-7 items-center justify-center rounded-full active:scale-90"
              style={{ backgroundColor: "rgba(0,0,0,0.6)" }} aria-label="?¥Î?ÏßÄ ?úÍ±∞">
            ...
            {imageFile && imageFile.size > 5 * 1024 * 1024 && (
              <div className="absolute bottom-8 left-2 flex items-center gap-1 rounded px-2 py-0.5 text-[10px]"
                ...
                <AlertCircle className="h-3 w-3" /> 5MB Ï¥àÍ≥º ???ÑÏÜ°???§Ìå®?????àÏäµ?àÎã§
              </div>
            )}
          </div>
        )}
      ...
      <div className="relative">
        <textarea ref={textareaRef} value={message} onChange={e => { setMessage(e.target.value); draft.onChange(e.target.value); }}
          placeholder="Î≥¥ÎÇº Î©îÏãúÏßÄÎ•??ÖÎ†•?òÏÑ∏??.." rows={5} maxLength={MAX_CHARS}
          style={{ backgroundColor: "var(--tg-theme-secondary-bg-color, #232e3c)", color: "var(--tg-theme-text-color)" }}
          className="w-full rounded-xl px-4 py-3 text-sm outline-none resize-none" aria-label="Î∞úÏÜ° Î©îÏãúÏßÄ" />
        ...
        <div className="flex items-center justify-between rounded-xl px-4 py-3"
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs" style={{ color: "var(--tg-theme-text-color)" }}>
              ?úÎç§ ?µÏû• {replyMacroActive ? "ÏºúÏßê" : "Í∫ºÏßê"}
            </span>
            {replyMacroMessage && (
      ...
      <motion.button
        onClick={() => confirm("Î∞úÏÜ° ?ïÏù∏", handleSend, `${selectedAccountIds.length}Í∞?Í≥ÑÏ†ï ¬∑ ${selectedGroupIds.length || "?ÅÏúÑ 5"}Í∞?Í∑∏Î£π${imageFile ? " ¬∑ ?¥Î?ÏßÄ ?¨Ìï®" : ""}?ºÎ°ú Î∞úÏÜ°?©Îãà??)}
        ...
        aria-label={sending ? "Î∞úÏÜ° Ï§? : sent ? "Î∞úÏÜ° ?ÑÎ£å" : "Î∞úÏÜ°?òÍ∏∞"}>
        <AnimatePresence mode="wait">
          {sending ? (
            ...
            <motion.div key="send" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center gap-2">
              ...
              {`Î∞úÏÜ° (${selectedAccountIds.length}Í∞?Í≥ÑÏ†ï)`}
            </motion.div>
          )}
        </AnimatePresence>
      </motion.button>
      ...
      {selectedGroupIds.length === 0 && (
        <p className="text-xs text-center" style={{ color: "var(--tg-theme-hint-color, #708499)" }}>Í∑∏Î£π???†ÌÉù?òÏ? ?äÏúºÎ©??ÅÏúÑ 5Í∞?Í∑∏Î£π???êÎèô Î∞úÏÜ°?©Îãà??/p>
      )}
      {ConfirmDialog}
    </div>
  );

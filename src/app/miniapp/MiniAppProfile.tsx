...
import { User, Settings, LogOut, ChevronRight, Star, Bell, Globe, Info, ExternalLink, Activity, CheckCircle2, AlertTriangle, XCircle, RefreshCw, Plus } from "lucide-react";
import { clearAll } from "@/lib/auth";
import { SITE } from "@/lib/site";
import { fetchAuthMe } from "@/lib/api";
import { fetchAccountHealthScore } from "@/lib/api-miniapp";
import Image from "next/image";
import { AccountStatusDot, AccountStatusLabel } from "@/components/ui/AccountStatusIndicator";
import { ThemeQuickToggle } from "@/components/ui/ThemeQuickToggle";
import { FontSizeControl } from "@/hooks/useAppFontScale";
import { PushSettingsPanel } from "@/components/ui/PushSettingsPanel";
import { useToastStore } from "@/components/ui/GlobalToast";
import { DataUsagePanel } from "@/components/ui/DataUsagePanel";
  ...
  return (
    <div className="pb-4">
      <div className="flex flex-col items-center py-8 px-4">
        ...
        <h2 className="text-lg font-bold">{user?.first_name || "?¬ìš©??}</h2>
      ...
      {accounts.length > 0 && (
        <div className="mx-4 mb-4 rounded-2xl overflow-hidden" style={{ backgroundColor: "var(--tg-theme-section-bg-color, #232e3c)" }}>
          <div className="flex items-center justify-between px-4 py-3 border-b" style={{ borderColor: "var(--tg-theme-section-separator-color, #3a4a5a)" }}>
            <div className="flex items-center gap-2"><Activity className="h-4 w-4" style={{ color: "var(--tg-theme-button-color, #5288c1)" }} /><span className="text-sm font-semibold">ê³„ì • ê±´ê°•</span></div>
            <span className="text-xs" style={{ color: "var(--tg-theme-hint-color, #708499)" }}>{stats.todaySent}ê±?/ ?¤ëŠ˜</span>
          </div>
          <div className="divide-y" style={{ borderColor: "var(--tg-theme-section-separator-color, #3a4a5a)" }}>
            {accounts.map(acc => (
              <div key={acc.id} className="flex items-center gap-3 px-4 py-3">
                <AccountStatusDot status={acc.status === "healthy" ? "active" : acc.status === "warning" ? "warning" : "error"} size="lg" />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2"><span className="text-sm font-mono truncate">{acc.phone}</span><AccountStatusLabel status={acc.status === "healthy" ? "active" : acc.status === "warning" ? "warning" : "error"} /></div>
                  <div className="flex items-center gap-2 mt-1">
                    <div className="h-1.5 rounded-full flex-1 overflow-hidden bg-gray-700">
                    ...
                    <span className="text-[10px] w-6 text-right font-medium" style={{ color: "var(--tg-theme-hint-color)" }}>{acc.healthScore}</span>
                  </div>
                </div>
              </div>
            ))}
      ...
      <div className="mx-4 mb-4 rounded-2xl overflow-hidden" style={{ backgroundColor: "var(--tg-theme-section-bg-color, #232e3c)" }}>
        <div className="px-4 py-3 border-b" style={{ borderColor: "var(--tg-theme-section-separator-color, #3a4a5a)" }}>
          <PushSettingsPanel />
        </div>
      </div>
      ...
      <div className="mx-4 mb-4 rounded-2xl overflow-hidden" style={{ backgroundColor: "var(--tg-theme-section-bg-color, #232e3c)" }}>
        <div className="flex items-center justify-between px-4 py-3"><span className="text-xs font-semibold">?”ë©´ ?¤ì •</span><div className="flex items-center gap-3"><ThemeQuickToggle /><FontSizeControl /></div></div>
      </div>

      <div className="mx-4 rounded-2xl overflow-hidden" style={{ backgroundColor: "var(--tg-theme-section-bg-color, #232e3c)" }}>
        <MenuItem icon={<User className="h-5 w-5" />} label="ê³„ì • ê´€ë¦? href="/app" />
        <div className="mx-4 h-px" style={{ backgroundColor: "var(--tg-theme-section-separator-color, #3a4a5a)" }} />
        <MenuItem icon={<Settings className="h-5 w-5" />} label="?„ì²´ ?€?œë³´?? value="Web" href="/app" />
        <div className="mx-4 h-px" style={{ backgroundColor: "var(--tg-theme-section-separator-color, #3a4a5a)" }} />
        <MenuItem icon={<Star className="h-5 w-5" />} label="?”ê¸ˆ?? value={planName} href="/pricing" />
        <div className="mx-4 h-px" style={{ backgroundColor: "var(--tg-theme-section-separator-color, #3a4a5a)" }} />
        <MenuItem icon={<Globe className="h-5 w-5" />} label="??ë²„ì „ ?´ê¸°" href={SITE.app} />
      </div>

      <div className="mx-4 mt-4 rounded-2xl overflow-hidden" style={{ backgroundColor: "var(--tg-theme-section-bg-color, #232e3c)" }}>
        <MenuItem icon={<Info className="h-5 w-5" />} label="ë²„ì „ ?•ë³´" value="v0.1.0" />
        <div className="mx-4 h-px" style={{ backgroundColor: "var(--tg-theme-section-separator-color, #3a4a5a)" }} />
        <MenuItem icon={<ExternalLink className="h-5 w-5" />} label="ë¬¸ì˜?˜ê¸°" href={`https://t.me/${SITE.support?.telegram?.replace("@", "") || "telemon"}`} />
      </div>

      <div className="mx-4 mt-4 rounded-2xl overflow-hidden" style={{ backgroundColor: "var(--tg-theme-section-bg-color, #232e3c)" }}>
        <MenuItem icon={<LogOut className="h-5 w-5" />} label="ë¡œê·¸?„ì›ƒ" danger onClick={handleLogout} />
      </div>
    </div>
  );
});

...
import { Send, CheckCircle2, Coins, Clock, Users, Loader2, TrendingUp, Plug, Plus, RefreshCw, CircleAlert, WifiOff, ArrowRight } from "lucide-react";
...
const StatCard = memo(function StatCard({ type, value, onClick }: { type: string; value: string; onClick?: () => void }) {
  const configs: Record<string, { icon: typeof Send; label: string; bg: string; iconBg: string; tab?: string }> = {
    activeAccounts: { icon: Users, label: "활성 계정", bg: "from-emerald-600/20 to-emerald-800/10", iconBg: "bg-emerald-500", tab: "profile" },
...
export const MiniAppDashboard = memo(function MiniAppDashboard() {
  const [state, setState] = useState({ tokenBalance: 0, activeAccounts: 0, queueCount: 0, todayTotal: 0, accounts: [] as AccountItem[], recentBroadcasts: [] as BroadcastItem[], lastUpdated: null as Date | null, error: false, online: true });
  ...
  useEffect(() => {
    relativeTimer.current = setInterval(() => setState(prev => ({ ...prev, lastUpdated: prev.lastUpdated })), 10000);
    ...
    <div className="p-4 pb-8 space-y-4 max-w-2xl mx-auto" onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd}>
      ...
      <div className="rounded-2xl p-4" style={{ backgroundColor: "var(--tg-theme-section-bg-color, #232e3c)" }}>
        ...
        {state.accounts.length === 0 ? <EmptyAccountState onConnect={handleConnectAccount} /> : (
          <div className="space-y-1">
            ...
            <button onClick={handleConnectAccount} className="flex w-full items-center justify-center gap-1.5 mt-2 py-2.5 rounded-xl text-xs font-medium active:scale-[0.98]"
      ...
      {state.lastUpdated && (
        <div className="flex items-center justify-center gap-1.5">
          ...
          <p className="text-[10px]" style={{ color: "var(--tg-theme-hint-color, #708499)" }}>{relativeTime} 갱신</p>
        </div>
      )}
    </div>
  );

...
export const metadata: Metadata = {
  title: "TeleMon | 텔레그램 자동화 매크로 플랫폼",
  description: "Telegram 계정 관리, 자동 응답(FAQ 매크로), 예약 발송을 하나의 대시보드에서! 코딩 없이 텔레그램을 자동화하세요.",
  keywords: "텔레그램, 텔레그램 매크로, 텔레그램 자동응답, 텔레그램 발송, FAQ 봇, Telegram 자동화",
};
        ...
        <div className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
          <div className="grid gap-10 md:grid-cols-4">
            <div className="space-y-4">
              ...
              <p className="text-sm text-app-text-secondary leading-relaxed">
                텔레그램 자동화의 모든 것.<br />하나의 대시보드로 관리하세요.
              </p>
            </div>
            {[
              { title: "서비스", links: [["기능 소개", "/features"], ["요금제", "/pricing"], ["API 키 발급", "/get-api-key"]] },
              { title: "문서", links: [["API 문서", "#"], ["사용 가이드", "#"], ["자주 묻는 질문", "#faq"]] },
              { title: "문의", links: [["이메일", "mailto:contact@telemon.io"], ["카카오톡", "#"], ["텔레그램", "#"]] },
            ].map((section) => (
              <div key={section.title}>
                <h3 className="mb-4 text-sm font-semibold text-app-text tracking-wide">{section.title}</h3>
                <ul className="space-y-3">
                  {section.links.map(([label, href]) => (
                    <li key={label}>
                      <Link href={href} className="text-sm text-app-text-secondary hover:text-app-text transition-colors">
                        {label}
                      </Link>
                    </li>
                  ))}
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            <p className="text-sm text-app-text-muted">
              &copy; 2026 TeleMon. All rights reserved.
            </p>
            <div className="flex items-center gap-6 text-sm text-app-text-muted">
              <Link href="#" className="hover:text-app-text transition-colors">이용약관</Link>
              <Link href="#" className="hover:text-app-text transition-colors">개인정보처리방침</Link>
            </div>
          </div>
        </div>
      </footer>

...
function DistributorsContent() {
  ...
  function setMemos(next: Record<string, string>) {
    setMemosState(next);
    try { localStorage.setItem("distributor-memos", JSON.stringify(next)); } catch (e) { console.warn('Unhandled error in page', e) }
  }
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      toast("error", "Ï¥ùÌåê ?ïÎ≥¥Î•?Î∂àÎü¨?§Ï? Î™ªÌñà?µÎãà??");
    } finally {
      setLoading(false);
    }
  }, [toast]);
  ...
  function exportDistributorsCsv(data: Distributor[]) {
    const headers = ["?ÑÌôîÎ≤àÌò∏","ÏΩîÎìú","?†Ïπò?∏Ïõê","?ÑÏ†ÅÎß§Ï∂ú","Ïª§Î???,"ÏßÄÍ∏âÏôÑÎ£?,"?ÅÌÉú","?òÏàòÎ£åÏú®"];
    const rows = data.map(d => [d.phone, d.referral_code, d.referral_count, d.total_revenue, d.total_commission, d.total_payout, d.status === "suspended" ? "?ïÏ?" : "?úÏÑ±", d.commission_rate_override != null ? `${(d.commission_rate_override * 100).toFixed(1)}%` : "?êÎèô"]);
  ...
  async function handleSuspend(tenantId: string, currentStatus: string, reason: string) {
    if (!reason.trim()) {
      toast("error", "?ïÏ? ?¨Ïú†Î•??ÖÎ†•?òÏÑ∏??);
      return;
    }
    setSuspending((prev) => ({ ...prev, [tenantId]: true }));
    try {
      await suspendDistributor(tenantId, reason, currentStatus !== "suspended");
      toast("success", currentStatus === "suspended" ? "Ï¥ùÌåê??Î≥µÍµ¨?òÏóà?µÎãà??" : "Ï¥ùÌåê???ïÏ??òÏóà?µÎãà??");
      await load();
  ...
  async function handleApprovePayout(payoutId: string) {
    try {
      ...
      toast("success", "ÏßÄÍ∏âÏù¥ ?πÏù∏?òÏóà?µÎãà??");
      await load();
    } catch {
      toast("error", "?πÏù∏ ?§Ìå®");
    }
  ...
  async function handleRejectPayout(payoutId: string) {
    ...
    if (!reason?.trim()) {
      toast("error", "Í±∞Ï†à ?¨Ïú†Î•??ÖÎ†•?òÏÑ∏??);
      return;
    }
    try {
      await rejectPayout(payoutId, reason);
      toast("success", "ÏßÄÍ∏âÏù¥ Í±∞Ï†à?òÏóà?µÎãà??");
      await load();
      setRejectReasons((prev) => ({ ...prev, [payoutId]: "" }));
    } catch {
      toast("error", "Í±∞Ï†à Ï≤òÎ¶¨ ?§Ìå®");
    }
  }
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-5 p-4 sm:p-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-base font-bold text-app-text">Ï¥ùÌåê Í¥ÄÎ¶?/h1>
          <p className="text-xs text-app-text-muted mt-0.5">Ï∂îÏ≤ú??Ï¥ùÌåê??Í¥ÄÎ¶¨ÌïòÍ≥?Ïª§Î??òÏùÑ ?ïÏÇ∞?©Îãà??/p>
        </div>
        <div className="flex items-center gap-2">
          <Button variant="primary" size="sm" onClick={handleBatchProcess}>
            <DollarSign className="h-3.5 w-3.5" /> ?ºÍ¥Ñ ?ïÏÇ∞ ?ùÏÑ±
          </Button>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        <SummaryCard icon={Users} label="Ï¥ùÌåê ?? value={totalDistributors} color="text-blue-500" onClick={() => toggleSort(null)} />
        <SummaryCard icon={TrendingUp} label="?†Ïπò ?∏Ïõê" value={totalReferrals} color="text-emerald-500" onClick={() => toggleSort("referral_count")} />
        <SummaryCard icon={DollarSign} label="Ïª§Î???Ï¥ùÏï°" value={totalCommission.toLocaleString()} color="text-violet-500" suffix="?? onClick={() => toggleSort("total_commission")} />
        <SummaryCard icon={CheckCircle2} label="ÏßÄÍ∏??ÑÎ£å" value={totalPayout.toLocaleString()} color="text-green-500" suffix="?? onClick={() => toggleSort("total_payout")} />
        <SummaryCard icon={Ban} label="?ïÏ?" value={suspendedCount} color="text-red-500" />
      </div>

      <Panel
        accent="violet"
        title={<div className="flex items-center gap-2"><Users className="h-4 w-4" /> Ï¥ùÌåê Î™©Î°ù</div>}
        description="?†Ïπò ?∏Ïõê?ò¬∑ÎàÑ??Îß§Ï∂ú¬∑Ïª§Î??òÏùÑ ?úÎàà???ïÏù∏?òÏÑ∏??
      >
        <div className="relative mb-3">
          ...
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)}
            placeholder="?ÑÌôîÎ≤àÌò∏ ?êÎäî Ï∂îÏ≤ú??ÏΩîÎìú Í≤Ä??
        ...
        {loading ? (
          ...
          <EmptyState icon={Users} title="Ï¥ùÌåê???ÜÏäµ?àÎã§" description="?ÑÏßÅ ?±Î°ù??Ï¥ùÌåê???ÜÏäµ?àÎã§." />
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-app-border text-app-text-muted">
                  <th className="px-2 py-2 text-left">?ÑÌôîÎ≤àÌò∏</th>
                  <th className="px-2 py-2 text-left">ÏΩîÎìú</th>
                  <th className="px-2 py-2 text-right">?†Ïπò{sortBy === "referral_count" ? <span className="ml-0.5">??/span> : ""}</th>
                  <th className="px-2 py-2 text-right">Îß§Ï∂ú{sortBy === "total_revenue" ? <span className="ml-0.5">??/span> : ""}</th>
                  <th className="px-2 py-2 text-right">Ïª§Î???sortBy === "total_commission" ? <span className="ml-0.5">??/span> : ""}</th>
                  <th className="px-2 py-2 text-right">ÏßÄÍ∏?sortBy === "total_payout" ? <span className="ml-0.5">??/span> : ""}</th>
                  <th className="px-2 py-2 text-center">Î©îÎ™®</th>
                  <th className="px-2 py-2 text-center">?ÅÌÉú</th>
                  <th className="px-2 py-2 text-center">?òÏàòÎ£åÏú®</th>
                  <th className="px-2 py-2 text-center">Í¥ÄÎ¶?/th>
                </tr>
              </thead>
                  ...
                  <tr key={d.tenant_id} className="hover:bg-app-card-hover/50 transition-colors">
                    ...
                    <td className="px-2 py-2.5">
                      <input type="text" value={memos[d.tenant_id] ?? ""}
                        ...
                        placeholder="Î©îÎ™®" className="w-24 rounded border border-app-border/60 bg-app-bg/50 px-1 py-0.5 text-[10px] text-app-text placeholder:text-app-text-subtle outline-none" />
                    </td>
                    <td className="px-2 py-2.5 text-center">
                      <Badge tone={d.status === "suspended" ? "danger" : "success"}>
                        {d.status === "suspended" ? "?ïÏ?" : "?úÏÑ±"}
                      </Badge>
                    </td>
                    <td className="px-2 py-2.5 text-center">
                      <div className="flex items-center gap-1 justify-center">
                        <input type="number" step="0.05" min="0" max="1"
                          ...
                          placeholder={d.commission_rate_override != null ? `${(d.commission_rate_override * 100).toFixed(1)}%` : "?±Í∏â ?êÎèô"}
                    ...
                    <td className="px-2 py-2.5 text-center">
                      <div className="flex items-center gap-1 justify-center">
                        <input type="text" value={suspendReason[d.tenant_id] ?? ""}
                          ...
                          placeholder="?ïÏ? ?¨Ïú†" className="w-20 rounded border border-app-border bg-app-bg px-1 py-0.5 text-[10px] text-app-text outline-none" />
                        <button onClick={() => handleSuspend(d.tenant_id, d.status, suspendReason[d.tenant_id] ?? "")}
                          ...
                          {suspending[d.tenant_id] ? "..." : d.status === "suspended" ? "Î≥µÍµ¨" : "?ïÏ?"}
                        </button>
                      </div>
                    </td>
                  </tr>
      ...
      <Panel
        accent="amber"
        title={<div className="flex items-center gap-2"><DollarSign className="h-4 w-4" /> ÏßÄÍ∏??ÄÍ∏?Î™©Î°ù</div>}
        description="?òÎèô Í≤Ä????ÏßÄÍ∏??πÏù∏ ?êÎäî Í±∞Ï†à Ï≤òÎ¶¨"
      >
        <div className="mb-3 flex gap-1.5">
          {["all","today","7d","30d"].map((key) => (
            <button key={key} onClick={() => setDateRange(key)}
              ...
              {key === "all" ? "?ÑÏ≤¥" : key === "today" ? "?§Îäò" : key === "7d" ? "7?? : "30??}
            </button>
          ))}
        </div>
        {payoutsLoading ? (
          ...
          <EmptyState icon={CheckCircle2} title="ÏßÄÍ∏??ÄÍ∏??ÜÏùå" description="Î™®Îì† ?ïÏÇ∞??Ï≤òÎ¶¨?òÏóà?µÎãà??" />
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-app-border text-app-text-muted">
                  <th className="px-2 py-2 text-left">Ï¥ùÌåê</th>
                  <th className="px-2 py-2 text-right">Í∏àÏï°</th>
                  <th className="px-2 py-2 text-right">?îÏ≤≠??/th>
                  <th className="px-2 py-2 text-center">Í±∞Ï†à ?¨Ïú†</th>
                  <th className="px-2 py-2 text-center">?ëÏóÖ</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-app-border">
                {filteredPayouts.map((p) => (
                  <tr key={p.id} className="hover:bg-app-card-hover/50 transition-colors">
                    ...
                    <td className="px-2 py-2.5 text-right font-semibold text-violet-500">{p.amount.toLocaleString()}??/td>
                    ...
                    <td className="px-2 py-2.5 text-center">
                      <input type="text" value={rejectReasons[p.id] ?? ""}
                        ...
                        placeholder="Í±∞Ï†à ???¨Ïú† ?ÖÎ†•" className="w-32 rounded border border-app-border bg-app-bg px-1.5 py-0.5 text-[10px] text-app-text outline-none" />
                    </td>
                    <td className="px-2 py-2.5 text-center">
                      <div className="flex items-center justify-center gap-1">
                        <button onClick={() => handleApprovePayout(p.id)}
                          ...
                          <CheckCircle2 className="h-3 w-3" /> ?πÏù∏
                        </button>
                        <button onClick={() => handleRejectPayout(p.id)}
                          ...
                          <XCircle className="h-3 w-3" /> Í±∞Ï†à
                        </button>
                      </div>
                    </td>
                  </tr>

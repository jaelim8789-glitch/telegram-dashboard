...
function getMockResponse(text: string): string {
  if (text.includes("통계") || text.includes("발송") || text.includes("성공률")) {
    return `오늘 발송 통계를 확인했습니다:\n\n📊 **총 발송:** 1,234건\n✅ **성공:** 1,211건 (98.2%)\n❌ **실패:** 23건 (1.8%)\n\n**상위 발송 계정:**\n1. 24시 자동응답 - 234건\n2. 프로모션 계정 A - 201건\n3. SNS 통합 관리 - 167건\n\n발송 성공률이 매우 양호합니다. 다만 뉴스레터 발송 계정과 파트너사 B 계정에서 오류가 감지되었으니 확인이 필요합니다.`;
  }
  ...
  if (text.includes("스팸") || text.includes("차단")) {
    return `오늘 스팸 차단 현황:\n\n🛡️ **총 차단: 47건**\n\n📊 차단 유형:\n- 피싱 링크: 18건\n- 스팸 키워드: 22건\n- 의심 계정: 7건\n\n모든 차단 건은 [관리자 로그](https://app.telemon.online/admin/logs)에서 확인 가능합니다.`;
  }
...
export function AiAssistantPageClient() {
  ...
  const handleSummaryClick = useCallback((id: string, title: string) => {

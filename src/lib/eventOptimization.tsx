...
class EventOptimizer {
  ...
  // Ï¶âÏãú ?§Ìñâ Í∞Ä?•Ìïú ?îÎ∞î?¥Ïã± (leading edge)
  ...
  public optimizeScrollHandler(
    ...
    options?: { 
      ...
      throttle?: number; 
      passive?: boolean; 
      useRAF?: boolean // requestAnimationFrame ?¨Ïö© ?¨Î?
    }
  ): EventListener {
  ...
  public optimizeMouseMoveHandler(
    ...
    options?: { 
      throttle?: number; 
      sampleRate?: number; // ?òÌîåÎß?ÎπÑÏú® (?? 0.5??50%Îß?Ï≤òÎ¶¨)
    }
  ): EventListener {
  ...
  public optimizeKeyboardHandler(
    ...
    // ?ÖÎ†• ?¥Î≤§?∏Îäî ?ºÎ∞ò?ÅÏúºÎ°??îÎ∞î?¥Ïã±?????ÅÏ†à??
    if (debounce) {
      optimizedHandler = this.debounce(optimizedHandler as any, debounce) as EventListener;
    }
...
export function useEventOptimization() {
  ...
  // Ïª¥Ìè¨?åÌä∏ ?∏Îßà?¥Ìä∏ ??ÏµúÏ†Å???ïÎ¶¨
  useEffect(() => {
    return () => {
      // ??Ïª¥Ìè¨?åÌä∏?êÏÑú ?ùÏÑ±??ÏΩúÎ∞±?§Îßå ?ïÎ¶¨

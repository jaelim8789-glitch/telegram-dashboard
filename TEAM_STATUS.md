...
**발동 조건:** 누군가 TEAM_STATUS.md 상단의 `⚠️ EMERGENCY STOP` 마커를 활성화하면 전원 push 중지.

**절차:**
1. TEAM_STATUS.md 최상단에 `⚠️ EMERGENCY STOP — [사유] — [시간]` 추가 후 커밋
2. 모든 에이전트/개발자는 push 전에 TEAM_STATUS.md 확인
3. 원인 해결 후 마커 제거

## 현재 작업 중 (2026-07-22)

담당자 J — 도구/자동감시 15종 구현 중
- tsc watch 데몬, JSX-in-.ts 검사, 배포 체크리스트, 포트 충돌 검사, post-merge, 워크트리 대시보드, 웹훅 mock, bisect, 인수인계, 주간리포트, 동시편집 경고

## 🔴🟡🟢 우선순위 규칙

| 표시 | 의미 | 적용 기준 |
|------|------|-----------|
| 🔴 | 긴급 | 빌드/배포 차단, 데이터 유실, 보안 |
| 🟡 | 중요 | 기능 동작에는 영향 있으나 우회 가능 |
| 🟢 | 개선 | 리팩토링, UI 개선, 문서화

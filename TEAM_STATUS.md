...
| 슈퍼 검색 (Command+K 통합검색) | OpenCode | 커밋완료 | - | 중복 CommandPalette 제거. 계정/그룹/템플릿/발송기록 API 디바운스 검색 |
| 대시보드 위젯 커스터마이징 + 발송통계차트 + 원클릭재발송 | OpenCode | 커밋완료 | - | 위젯 드래그순서변경/숨기기, LogTab Recharts 차트+재발송버튼 |
| 템플릿 버전 히스토리 + 발송시간대 히트맵 활성화 | OpenCode | 커밋완료 | - | localStorage 버전저장/복원, 7x24 히트맵 실시간데이터 |

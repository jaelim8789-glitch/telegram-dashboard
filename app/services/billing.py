...
async def confirm_usdt_payment(tenant_id: str, tx_hash: str) -> dict:
    ...
    async with async_session_maker() as db:
        ...
        await create_commission(
            ...
            source_payment_id=tx_hash,
            source_type="usdt",
            amount=max(1, get_plan_price_usdt(tenant.plan, "monthly") or get_plan_price_usdt(tenant.plan, "quarterly") or 0) * 100,
        )

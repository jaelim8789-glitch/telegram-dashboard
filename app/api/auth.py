...
async def verify_code(payload: VerifyCodeRequest, db: AsyncSession = Depends(get_db)):
    ...
    if payload.referral_code:
        ref_result = await db.execute(
            select(ReferralCode).where(ReferralCode.code == payload.referral_code)
        )
        ref_code = ref_result.scalar_one_or_none()
        if ref_code is None:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="유효하지 않은 추천인 코드입니다.",
            )
        if ref_code.owner_id == (tenant.id if tenant else None):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="자기 자신의 추천인 코드는 사용할 수 없습니다.",
            )
        referred_by_id = ref_code.id
    ...
    elif referred_by_id and not tenant.referred_by:
        tenant.referred_by = referred_by_id
        await db.flush()

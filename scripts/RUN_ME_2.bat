...
set LOGFILE=C:\Dev\TeleMon\scripts\master_result2.txt
echo ======================================== > %LOGFILE%
echo CONTINUED MERGE SCRIPT (after conflict fix) >> %LOGFILE%
...
echo --- Staging resolved files and committing merge --- >> %LOGFILE%
git add src/lib/accountLabels.ts src/components/sidebar/AccountCard.tsx >> %LOGFILE% 2>&1
git commit --no-edit -m "Merge feat/account-labels" >> %LOGFILE% 2>&1
if !ERRORLEVEL! equ 0 (
    echo [OK] feat/account-labels merge committed successfully >> %LOGFILE%
    echo [OK] feat/account-labels merged!
) else (
    echo [FAIL] feat/account-labels commit FAILED >> %LOGFILE%
    echo.
...
for %%b in (feat/account-search feat/keyboard-shortcuts feat/csv-export feat/tab-error-badge feat/message-templates) do (
    echo. >> %LOGFILE%
    ...
    if !ERRORLEVEL! equ 0 (
        ...
        echo [FAIL] %%b FAILED - check %LOGFILE%
...
if !ERRORLEVEL! equ 0 (
    ...
    echo [FAIL] TypeScript check FAILED - check %LOGFILE%
    pause
    exit /b 1
)
...
if !ERRORLEVEL! equ 0 (
    ...
    echo [FAIL] Build FAILED - check %LOGFILE%
    pause
    exit /b 1
)

#!/usr/bin/env bash
# check-env-sync.sh ??.env.example vs real .env ???ÑÎùΩ Í∞êÏ?
#
# Usage:
#   ./scripts/check-env-sync.sh                     # frontend Ï≤¥ÌÅ¨
#   ./scripts/check-env-sync.sh backend             # backend Ï≤¥ÌÅ¨
#   ./scripts/check-env-sync.sh --fix               # ?ÑÎùΩ?§Î? .env???êÎèô Ï∂îÍ? (Ï£ºÏÑù Ï≤òÎ¶¨)
#
# Exit code: 0 = OK, 1 = ?ÑÎùΩ ?àÏùå
...
echo "?†Ô∏è  ${#MISSING[@]} key(s) missing from $ENV_FILE:"
printf '   - %s\n' "${MISSING[@]}"
